...
import { buildSyntheticDataset, linearEvents, type SyntheticDataset, type SyntheticEventSpec } from "../../../server/researchEngine/testFixtures";
...
describe("ResearchEngine", () => {
  ...
  it("Dataset Version 非 READY → DATASET_VERSION_NOT_READY（BUILDING 版本不得用于研究）", async () => {
    ...
    const reader = new (await import("../../../server/researchEngine/datasetReader")).InMemoryResearchDatasetReader({
      context: ds.context,
      events: ds.events,
      paths: ds.paths,

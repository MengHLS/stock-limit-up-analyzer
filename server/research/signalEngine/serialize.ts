/**
 * STEP 13 / C-13.2 — Signal 与 Candidate 框架：运行记录序列化。
 *
 * 铁律（对齐 framework/serialization.ts）：
...
import type { CandidateEvaluationRun } from "./types";
import { assertValidCandidateEvaluationRun } from "./validate";
 ...
 */
export function computeCandidateEvaluationRunFingerprint(record: Omit<CandidateEvaluationRun, "fingerprint"> | CandidateEvaluationRun): string {
  const json = JSON.stringify(record, fingerprintReplacer);
  return createHash("sha256").update(json, "utf8").digest("hex");
}

/** 序列化运行记录（拒绝 NaN/Infinity；不含时间戳/随机数）。 */
export function serializeCandidateEvaluationRun(record: CandidateEvaluationRun): string {
  return JSON.stringify(record, strictReplacer);
}

/** 反序列化运行记录：结构复核 + fingerprint 完整性校验。 */
export function deserializeCandidateEvaluationRun(json: string): CandidateEvaluationRun {
  const parsed: unknown = JSON.parse(json);
  assertObject(parsed, "candidateEvaluationRun");
  const record = parsed as unknown as CandidateEvaluationRun;
  assertValidCandidateEvaluationRun(record);
  const recomputed = computeCandidateEvaluationRunFingerprint(record);

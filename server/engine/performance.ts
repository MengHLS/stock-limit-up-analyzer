/**
 * Backtest Core — Performance Analytics（绩效分析）。
 *
 * 职责：所有绩效指标统一从 equityCurve 与 trades 计算，绝不内联到策略/引擎。
 * 所有数学原语统一调用 shared/quant-stats，禁止重新实现 mean / std / Sharpe / CAGR。
 *
 * 口径约定（与 Step 1 统一）：
 *  - Sharpe = mean(dailyReturn) / sampleStdDev(dailyReturn) · √(annualizationFactor)
 *  - CAGR  = (endEquity / startEquity)^(annualizationFactor / n) − 1（几何年化，与 Sharpe 分离）
 */
...
import type { EquityPoint, PerformanceMetrics, Trade } from "./domain";

const round = (value: number, digits = 4) => Number(value.toFixed(digits));

/** 从权益曲线提取日收益率序列（跳过首点，收益率 = equity[i]/equity[i-1] − 1）。 */

...
import type {
  ObservationKind,
  PostEventAmplitudeCustomPayload,
} from "./result";
...
export default function PostEventAmplitudeStudyPage({ descriptor, outcome }: ExperimentPageProps) {
  ...
  const custom = (result.customPayload ?? {}) as Partial<PostEventAmplitudeCustomPayload>;
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-base">
            <Activity className="h-4 w-4" /> 首板后无涨跌停与振幅
          </CardTitle>
          <CardDescription className="text-xs">
            T+1..T+5 用 high/low 判断是否触及涨跌停；单日振幅 =
            `(high-low)/preClose`。T+5 决策后，从 T+6 开盘入场。
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-2 text-xs text-muted-foreground md:grid-cols-2">
          <p>
            无涨跌停 {counts?.noLimitCount ?? "—"}；
            至少一次涨跌停 {counts?.hadLimitCount ?? "—"}；
            T+6 不可买 {counts?.entryUnfillableCount ?? "—"}。
          </p>
          <p>
            后续视界 T+
            {custom.parameters?.forwardHorizons?.join(" / T+") ?? "—"}。
          </p>
          <p className="md:col-span-2">
            振幅分桶只用于描述，不代表最佳振幅区间；结果尚未做日期聚类 Bootstrap 或 Holdout。
          </p>
        </CardContent>
      </Card>

...
describe("selectFetchCells — 矩阵首屏取数上限", () => {
  const cells = Array.from({ length: 30 }, (_unused, i) => i + 1);

  it("默认只取前 MATRIX_INITIAL_FETCH 个（30 格 → 12 个请求，不是 30 个）", () => {
    const picked = selectFetchCells(cells, false);
    expect(picked.length).toBe(MATRIX_INITIAL_FETCH);
    expect(picked).toEqual(cells.slice(0, MATRIX_INITIAL_FETCH));
  });

  it("fetchAll = true 时取全部（用户显式要求，不是静默截断）", () => {
    expect(selectFetchCells(cells, true)).toEqual(cells);
  });

  it("格数少于上限时全取", () => {
    const few = cells.slice(0, 5);
    expect(selectFetchCells(few, false)).toEqual(few);
  });

  it("空集合返回空，不抛异常", () => {
    expect(selectFetchCells([], false)).toEqual([]);
    expect(selectFetchCells([], true)).toEqual([]);
  });

  it("非法 limit 回落到「不发请求」而不是全发", () => {
    expect(selectFetchCells(cells, false, 0)).toEqual([]);
    expect(selectFetchCells(cells, false, -3)).toEqual([]);
    expect(selectFetchCells(cells, false, Number.NaN)).toEqual([]);
  });

  it("纯函数：不改入参", () => {
    const input = cells.slice();
    selectFetchCells(input, false);
    expect(input).toEqual(cells);
  });

  it("上限是正的常量（防止被误改成 0 导致矩阵永不取数）", () => {
    expect(MATRIX_INITIAL_FETCH).toBeGreaterThan(0);
  });
});

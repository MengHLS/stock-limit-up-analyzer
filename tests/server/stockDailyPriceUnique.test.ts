...
import { duplicateStockDailyPriceIdsToRemove } from "../../server/db";
import { stockDailyPrices } from "../../drizzle/schema";

// 测试文件已收拢到 tests/，read() 的基准需回到原 server/ 目录（../../ = 仓库根，再进 server/）
const read = (rel: string) =>
  readFileSync(new URL(rel, new URL("../../server/", import.meta.url)), "utf8");

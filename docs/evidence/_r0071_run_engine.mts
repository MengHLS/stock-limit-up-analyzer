/**
 * RESEARCH-007.1 — 执行 Run 540001 并把结果落成可读清单（真实 TiDB）。
 *
 * 用法：npx tsx _r0071_run_engine.mts
 */
...
async function main(): Promise<void> {
  ...
  lines.push(JSON.stringify(execution, null, 2).slice(0, 6000));
  ...
  writeFileSync("_r0071_run_result.md", lines.join("\n"));
  console.log(`done in ${seconds}s`, "written _r0071_run_result.md");
  process.exit(0);
}

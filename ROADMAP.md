...
> 🔴 铁律：§44 为「覆盖式」状态区（**只保留最近 1 条「上轮实查」**，历史条目在 `ROADMAP-CHANGELOG.md`）；§44.5 为**未完成**队列（编号按「下一个未占用」，已用至 `9bu`）；§47 为 append-only 更新记录。
...
> 上轮实查：**2026-09-19 22:05 GMT+8 · 首页（`/`）第五轮：底部四个快捷入口卡片整体移除**（编号 `9bu`；用户口述「现在首页最下面有四个大的跳转按钮没用 去掉」）。**改动**：`client/src/pages/Dashboard.tsx` 删 `SHORTCUTS` 常量 + `Link` 卡片网格渲染块 + 已无引用的 `import { Link } from "wouter"`（`Card*` 系列仍被其余 17 处引用，不动）；`server/**` **零改动**（用户在用页面，按三门第 1 条只走 `client/**` HMR）。**实查判据**：无头 Chrome + CDP 真机量 DOM **PASS 54 / FAIL 0 / SKIP 0 ⇒ ALL PASS**（`_probe_homepage_rework.mjs` 原「快捷入口 4 个」**改写为反证断言**「0 张卡片 / 0 条跳转链」，实测 `{"links":0,"cards":0}`）；`tsc --noEmit` = **23 条错误、集合与基线逐字一致**（全在 parameterSearch / searchRobustness 在研区，`client/**` 零新增）；`pnpm run test:changed` = **8 失败文件 / 17 例**，失败**文件集合**与基线逐字一致（7 环境依赖 + `parameterSearchEffectiveness`）；`node scripts/checkEolDrift.mjs` = 0 漂移。四个路由与侧栏入口保持可达（只摘掉首页重复入口）。**顺带对账**：编号台账两处 `9br` → `9bu`（`9bs` = PARAMETER-001、`9bt` = PARAMETER-002 早前已在 `docs/parameter-search/` 等实占但未进台账）。
...
> 🔴 **编号台账（禁「末条 +1」）**：编号已用至 **`9bu`** ⇒ **下一个未占用 = `9bv`**。⚠️ **台账滞后已修（2026-09-19 / `9bu` 顺手对账）**：原两处写「用至 `9br`」，但 `9bs` = PARAMETER-001、`9bt` = PARAMETER-002 **已在其他文档实占**（`docs/parameter-search/PARAMETER-001-REPORT.md`、`PARAMETER-002-REPORT.md`、`docs/architecture/SYSTEM-BASELINE.md` v1.1.0/v1.1.1 增量、`CHANGE-AUDIT.md`）而未进本台账 ⇒ 本次按「**下一个未占用**」同步为 `9bu`（`9bu` 原仅为 `PLAN-STRATEGY-MODULE-EXTENSION-001` 的**建议号**、未分配，现由 HOMEPAGE-006 实占 ⇒ 该方案若获批准应取 `9bv`）。`9bs`/`9bt` 两条**不追补条目**（避免与那两轮的作者重复登记）。已完成的 `1~9` 与 `9a~9bh`、以及第 10~27 项中的 ✅ 已完成项，已**全部移入 `ROADMAP-CHANGELOG.md`**（原样搬运）。本项目历来按「**下一个未占用**」取号，**禁按「末条 +1」推算**。

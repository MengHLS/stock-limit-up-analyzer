...
> 🔴 铁律：§44 为「覆盖式」状态区（**只保留最近 1 条「上轮实查」**，历史条目在 `ROADMAP-CHANGELOG.md`）；§44.5 为**未完成**队列（编号按「下一个未占用」，已用至 `9cg`）；§47 为 append-only 更新记录。
...
> 上轮实查：**2026-09-20 · RESEARCH-EXPERIMENT-003（旧 Research 体系彻底删除）（用户规格）**（编号 `9cg`；`tsc --noEmit` **0 error**（69→41→11→1→0 五轮收敛）、**旧 Research 的两层实现整体删除**（`server/researchCore/**` 12 文件 + `server/researchEngine/**` 47 文件）、**旧 API 删除**（`researchEngine.*` 1251 行 / `researchPlanner.*` 897 行 / `researchRun.chainHealth`）、**DB 手工 migration `0046`**（5 张零行表 DROP + 12 张有历史行的表 RENAME 为 `archive_research_*`；`executed 12/skipped 5/pass true`，复跑 `executed 5/skipped 12/pass true`；执行后实查 0 残留 / 12 归档 / 4 张保留表原样）、**前端 10 条旧路由 + 19 个旧组件 + 侧栏 5 项删除**（「研究」分组 7→2）。🔴 **关键结构动作 = 删除 `server/research/index.ts` barrel**：它 `export *` 整个旧目录，而 `researchRunRouter` 与 (原) `researchRouter` 都从它取符号 ⇒ **只要它在，旧目录任何文件都在生产可达集里**；改为按文件显式导入后，删目录才真正等于「从依赖图上移除」。**搬迁（逻辑逐字未改）**：`researchCore/{types→vocabulary, candidates→candidateRules, conditions→conditionSet, serialization→jsonCodec}`、`researchEngine/{datasetReader→researchRuntime/datasetReader, columnProjection→researchRuntime/datasetColumns, types→researchRuntime/versionContext, planner/moduleRegistry→patternLibrary/moduleRegistry}`、候选仓储**重写**为 `research/candidateRepository.ts`（只服务一张表，**不提供 create**——旧 create 的两条父引用校验指向已归档表）。**Strategy Candidate（§4）判定为保留但去旧**：删除 `createFromConclusion`（Conclusion→Candidate 派生桥）、`deriveCandidateEvidence`（读 finding→analysis→moduleKey）、三个旧锚探测；`buildView` 的上游摘要如实返回 `null`且**不再**计入 `sourceMissing`。**Research Artifact（§5）判定为旧 Research 专属 ⇒ 删除**。**命名空间改名**：`research.*` → `strategyDomain.*`（server 文件改名 + 11 个前端文件 + 2 处类型索引）。🔴 **两个真踩**：① `applySqlMigration.mjs` 的 guard 语义是「目标已存在即跳过」⇒ 给 `DROP TABLE` 加 `@guard: table <要删的表>` 会让它**永远删不掉**（第一次实测 5 张表全 skip-exists，已改为不加 guard、靠 `IF EXISTS` 幂等）；② 误用 `git checkout -- drizzle/schema.ts` 覆盖掉 002 的**未提交**改动（`strategy_research_provenance` 的 5 新列 + 3 放宽），已按 0045 原文恢复并复检。报告 = `docs/research/RESEARCH-EXPERIMENT-003-final.md`；migration = `drizzle/0046_legacy_research_retire.sql`。**未做（如实登记）**：生产链真机 E2E 与「外部 AI 接入 E2E」本轮**未重跑**；`vocabulary.ts` 仍保留旧实体**类型声明**（无实现）；`patternLibrary` 的 `research` 投影已无运行时消费者 ⇒ 均列入 004。
...
> 🔴 **编号台账（禁「末条 +1」）**：编号已用至 **`9cg`** ⇒ **下一个未占用 = `9ch`**。已完成的 `1~9` 与 `9a~9bh`、以及第 10~27 项中的 ✅ 已完成项，已**全部移入 `ROADMAP-CHANGELOG.md`**（原样搬运）。本项目历来按「**下一个未占用**」取号，**禁按「末条 +1」推算**。

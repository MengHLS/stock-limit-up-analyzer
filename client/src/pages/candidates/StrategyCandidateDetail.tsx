...
import { PromotionEligibilityCard } from "@/components/research/PromotionEligibilityCard";
...
function CandidateDetailBody({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* FRONTEND-FINAL-001 §八 / P1-3 —— 转正资格**直接渲染在页面上**：
          不依赖「转正为策略」按钮的 disabled 态与 title 悬停提示（disabled 不触发
          onClick ⇒ 弹窗不打开 ⇒ 此前那段解释永远不可达）。位置定在候选概览之后、
          草图之前：先看清「为什么还不能转正」，再看草图细节。 */}
      <PromotionEligibilityCard vm={vm} raw={raw} />
...
export default function StrategyCandidateDetail() {
  ...
  const detail = trpc.strategyDomain.strategyCandidate.get.useQuery(
    { candidateId },
    { enabled: validId },
  );

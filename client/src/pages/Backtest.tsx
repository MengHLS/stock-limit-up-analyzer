...
function describePositionSizing(sizing: { strategy: "equal" | "scoreWeighted" | "fixedPercent"; fixedPositionPercent: number; initialCapital: number }) { if (sizing.strategy === "fixedPercent") return `固定单笔比例 —— 初始资金 ¥${formatMoney(sizing.initialCapital)} × ${sizing.fixedPositionPercent}%`; if (sizing.strategy === "scoreWeighted") return "评分加权 —— 可用现金 × 该笔策略分 ÷ 本批策略分合计（合计为 0 时退化为等权）"; return "等权分仓 —— 可用现金 ÷ 计划买入笔数"; }
/** 口径短标签（表格列头用；完整公式见表格合计行）。 */
function describePositionSizingShort(sizing: { strategy: "equal" | "scoreWeighted" | "fixedPercent"; fixedPositionPercent: number }) { if (sizing.strategy === "fixedPercent") return `固定单笔 ${sizing.fixedPositionPercent}%`; if (sizing.strategy === "scoreWeighted") return "评分加权"; return "等权分仓"; }
/** 高位连板降仓文案（规则口径唯一权威 = shared/boardHeightRisk，此处只做回显）。 */
function describeBoardHeightSizingRule(strategyKey: OrderStrategyKey, maxParticipatingBoards: number) { return strategyKey === "baseline" ? "原始策略作为对照基准，不施加高位连板仓位约束。" : `连板高度超过 ${maxParticipatingBoards} 板按风控不分配仓位；5 板 ×0.6、6 板及以上 ×0.3。`; }
/**
 * 「当前持仓」的金额与仓位占比 = 展示层**恒等变形**（价 × 量、量 ÷ 总权益），不含任何估计：
 *   成本金额 = 买入价 × 股数（不含交易费用）；持仓市值 = 截止日收盘价 × 股数；
 *   占总权益 = 持仓市值 ÷ 该策略回测截止日总权益（与「当前持仓」同一模拟器、同一截止日）。
 * 任一价格缺失时如实回显「待补」，不猜测价格。
 */
type PortfolioHoldingLike = { shares: number; entryPrice: number | null; valuationPrice: number | null };
function holdingCostAmount(holding: PortfolioHoldingLike): number | null { return holding.entryPrice !== null && holding.entryPrice > 0 && holding.shares > 0 ? holding.entryPrice * holding.shares : null; }
function holdingMarketAmount(holding: PortfolioHoldingLike): number | null { return holding.valuationPrice !== null && holding.valuationPrice > 0 && holding.shares > 0 ? holding.valuationPrice * holding.shares : null; }
function holdingWeightPercent(holding: PortfolioHoldingLike, equity: number | null): number | null { const market = holdingMarketAmount(holding); if (market === null || equity === null || !Number.isFinite(equity) || equity <= 0) return null; return (market / equity) * 100; }
  ...
  return <div className="mt-6 border-t border-violet-100 pt-4">
    ...
    <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
      {blocks.map((block) => <div key={block.key} data-full-cycle-risk-card={block.key} className="rounded-xl border border-violet-100 bg-violet-50/40 p-3">
        ...
        <p className="mt-3 text-[11px] leading-4 text-slate-500">最大收益日 {block.maxReturnDate ? formatDate(block.maxReturnDate) : "-"}；期末日 {block.currentDate ? formatDate(block.currentDate) : "-"}。</p>
      </div>)}
    </div>
  </div>;
}
...
function StrategyEvaluationTable({ title, description, experiments, outOfSample = false }: { title: string; description: string; experiments: EvaluationExperiment[] | WalkForwardExperiment[]; outOfSample?: boolean }) {
  return <div className="overflow-auto rounded-xl border border-slate-200">
    <div className="border-b border-slate-200 bg-slate-50 px-4 py-3"><h3 className="text-sm font-semibold text-slate-800">{title}</h3><p className="mt-1 text-xs leading-5 text-slate-500">{description}</p><p className="mt-1 text-[11px] leading-4 text-slate-400">已平仓胜率、盈亏比与「已出清」列只统计已平仓交易；回测结束仍持有的仓位计入累计收益、最大回撤与期末资金，但不计入胜率，也不计为失败交易。</p></div>
...
function FullOrdersSection({
  ...
  // 全部模拟订单分页：订单全集随查询一次取回（研究-legacy 全周期实验），此处只做前端切片，
  ...
  // 策略 / 状态 / 原因 / 关键词 / 排序 / 每页条数任一变更 ⇒ 回到第 1 页，避免停在越界页码上。
  useEffect(() => { setPage(1); }, [strategy, orderStatus, reason, keyword, sortKey, sortDirection, pageSize]);
  const totalPages = Math.max(1, Math.ceil(displayedOrders.length / pageSize));
  const currentPage = Math.min(Math.max(1, page), totalPages);
  const pageStart = (currentPage - 1) * pageSize;
  const pagedOrders = displayedOrders.slice(pageStart, pageStart + pageSize);
  ...
  return (
    <section data-all-simulated-orders className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-4 flex flex-col gap-4 border-b border-slate-100 pb-4">
        <div className="flex flex-wrap items-start gap-3">
          <div className="mr-auto">
            ...
            <p className="mt-1 text-xs text-slate-500">切换后展示对应全周期策略的全部订单；红涨绿跌。订单按页展示，默认每页 20 笔，可翻页或调整每页条数。</p>
          </div>
          ...
          {expectationTierSummary.map((item) => {
            ...
            return (
              <div key={item.tier} className="rounded-lg border border-slate-200 bg-slate-50/60 px-3 py-2">
                ...
                <p className="mt-1 text-xs text-slate-600">已出清平均收益 {item.averageNetReturn === null ? "-" : `${item.averageNetReturn.toFixed(2)}%`} · 已出清胜率 {item.winRate === null ? "-" : `${item.winRate.toFixed(1)}%`}</p>
              </div>
            );
          })}
        </div>
      ...
      <div data-orders-pagination className="mt-3 flex flex-wrap items-center gap-3 border-t border-slate-100 pt-3">
...
export default function BacktestPage() {  const [config, setConfig] = useState({
  ...
  // 回测区间（含边界）。默认起点固定为 2025-09-01，终点为今天；数据回填到 2019 后若全量加载会命中 489 万行价格并卡死，故必须限区间。
  const [dateRange, setDateRange] = useState<{ startDate?: string; endDate?: string }>(() => {
    const toIso = (d: Date) => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
    return { startDate: "2025-09-01", endDate: toIso(new Date()) };
  });
  ...
  // 该策略「回测截止日总权益」= 研究-legacy 全周期实验的 finalCapital，用作当前持仓市值占比的分母
  // （与「当前持仓」同一模拟器、同一截止日，避免跨模拟器混算）。
  const selectedPortfolioEquity = downsideRiskResearch?.fullCycle.experiments.find((experiment) => experiment.key === selectedStrategyPortfolio?.key)?.realisticSimulation.finalCapital ?? null;
  const plannedSizing = selectedStrategyPortfolio?.plannedPositionSizing ?? null;
  const holdingsSummary = summarizeHoldings(selectedStrategyPortfolio?.currentHoldings ?? [], selectedPortfolioEquity);
  ...
  // 折线图下方的收益/回撤区块：回撤三项取服务端权威值，收益两项由权益曲线恒等派生（含初始资金校验）。
  const fullCycleRiskBlocks = useMemo(() => buildFullCycleRiskBlocks(fullCycleExperiments.map((item) => ({
    key: item.key,
    label: item.label,
    color: strategyColors[item.key as OrderStrategyKey],
  ...
  return <main className="pb-12 text-slate-900">
    ...
    <div className="mx-auto max-w-7xl space-y-5 px-4 pt-5 sm:px-6">
      ...
      {isLoading || !simulation ? <div className="flex min-h-72 items-center justify-center rounded-2xl border border-slate-200 bg-white"><Loader2 className="h-7 w-7 animate-spin text-sky-600" /></div> : <>
        {/* 统一策略评价已迁至「策略对比」页签（见上方 data-strategy-evaluation 渲染）。 */}
...
function HistorySection() {
  ...
  const compareMetrics: Array<{ label: string; value: (s: Record<string, unknown> | null) => string; tone?: (s: Record<string, unknown> | null) => string }> = [
    ...
    { label: "已平仓胜率", value: (s) => num(s?.realisticWinRate, "%") },

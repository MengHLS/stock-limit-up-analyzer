...
> 🔴 铁律：§44 为「覆盖式」状态区（**只保留最近 1 条「上轮实查」**，历史条目在 `ROADMAP-CHANGELOG.md`）；§44.5 为**未完成**队列（编号按「下一个未占用」，已用至 `9co`）；§47 为 append-only 更新记录。
...
> 上轮实查：**2026-09-21 · `9cn`（`BD-24`「跑完 first-board-pullback 在『回测历史』里看不到」—— 长算后留档写库撞死连接）**（编号 `9cn` ⇒ **下一个未占用 = `9co`**；本轮全部闸门**实跑**：`tsc --noEmit` **0 error**；定向 `vitest` **4 文件 / 23 例全绿**；🔴 **修前 / 修后各真跑一次**（同一条探针 `_probe_fbp_looprun_repro2.mts`、同一组入参）：修前 **588881 ms**（9.81 分钟）后 `fbpRows` **0** ⇒ 库里恒缺这条（3 次 INSERT + 紧随的列表 SELECT 全 `read ECONNRESET` errno −4077）；修后 **571761 ms**（9.53 分钟）后 `fbpRows` **1** ⇒ **「回测历史」出现该行**）。**① 双层根因（都靠实测定案）**：⒜ **池阈值 > 对端窗口** —— 实测链路空闲窗口 ∈ **(240, 330] s**（A/B 探针：同一条 socket 空闲 240s 仍存活、330s 起全部 `ECONNRESET`），而池的 `idleTimeout` 是 **600s** ⇒ 一次 **588,881 ms** 的长算（其间不再发 SQL）**恰好躲过回收** ⇒ 写库时取到的全是「看似可用」的半开死 socket；⒝ **保活没生效** —— `enableKeepAlive: true` 但 `keepAliveInitialDelay: 0`，而 Node 文档明写 `initialDelay = 0` 表示*沿用系统默认*（本机 `HKLM\…\Tcpip\Parameters` 未配 `KeepAliveTime` ⇒ **2 小时**）⇒ 死连接既不被保活检出、也不会被提前移除。**② 最小修复（零 DDL / 零迁移 / 零新依赖 / 零新端点）**：⒜ `server/db.ts` 的 `resolveIdleTimeoutMs()` 默认 **600_000 → 180_000**，判据是**派生的**：必须 *严格小于* 实测窗口下界 240s（并新增导出常量 `MEASURED_DB_IDLE_WINDOW_LOWER_BOUND_MS = 240_000`）；⒝ `researchRunRouter#persistClosedLoopBacktestRun` 的**留档重试**从 **3 → 6 次**（派生自实测：该次运行结束池内**至少 4 条**已死 ⇒ 3 次必然不够），并**只在瞬时错误上重试**（非瞬时立即放弃，复用只读路径的同一份特征表 `readRetry.ts#isTransientReadError`，不再白等 N × 19.28s），函数改为可注入依赖以便单测；⒞ **保活初始延迟维持 `0`**（**不是**修复项）。**③ 🔴 能力边界（修后真跑实测，纠正了我先入为主的因果）**：回收器**只处理自由队列**、判据是 `Date.now() - _freeConnections.get(0).lastActiveTime > idleTimeout`，而 `lastActiveTime` 是在 `release()` 里刷新的（源码 = `mysql2/lib/base/pool.js:198-210` + `.../base/pool_connection.js:23-30`）⇒ **「借出跨越掐断窗口、之后才还回池」的连接对它永久不可见**。实测印证：写库边界的陈旧连接由修前 **≥4 条**降到修后 **1 条**（`idleTimeout` 确实生效），**但第 1 次尝试仍撞死连接**（白等 **19.28 s** 的 TCP 重传超时），靠第 2 次换到新连接（**3.86 s**）才成功 ⇒ **承重项是留档重试预算，`idleTimeout` 只负责把陈旧连接压进预算之内**。注释里原先那句「死连接在被掐之前就被回收」是**过强**表述，已按实测改写为「能缩小、不能清零」。**④ 🔴 一次被证伪的「修复」（本轮最有价值的负面证据）**：曾把 `keepAliveInitialDelay` 改成 **30_000**（假设「保活能防掐断」）⇒ ⒜ A/B 探针**证伪**：`30_000` 组在 330/360/480s **同样全部失败**（保活挡不住链路掐断）；⒝ 真跑一次后**进程直接退出** —— 失败形态由「半开挂 19.3~20.0s 后 `ECONNRESET`」变成「**0ms** 已关闭 socket」⇒ `EPIPE`（`writeAfterFIN`）触发 mysql2 `_handleFatalError → _notifyError` **二次** `emit('error')`，而 `pool_connection.js` 只用 **`once('error')`** ⇒ **unhandled 'error' event**（`code: 'EPIPE', fatal: true`）。证据：`_probe_fbp_looprun_repro2.keepalive-trial.out.txt` ⇒ **已按实测回退**并在 `db.ts` 就地写下「别改成有限值」的红线。**⑤ 交付面**：生产 **2** 文件（`server/db.ts` / `server/researchRunRouter.ts`）、测试 **2** 新增（`tests/server/dbPoolConfig.test.ts` 3 例 + `tests/server/closedLoopBacktestRun/persistRetry.test.ts` 5 例）、证据 **4** 组（A/B 探针 `_probe_db_keepalive_ab.mts` → `.out.json/.out.txt/.stdout.txt`；修前证据 `_probe_fbp_looprun_repro2.prefix-BD24.out.{json,txt}`；保活试跑崩溃证据 `_probe_fbp_looprun_repro2.keepalive-trial.out.txt`；修后 `_probe_fbp_looprun_repro2.out.{json,txt}`）、文档 4（evidence README / CHANGE-AUDIT / `ROADMAP.md` / `ROADMAP-CHANGELOG.md`）+ 记忆 2（`MEMORY.md` 第三次超限 ⇒ 按既有范式做「**原文下移 #5**」，8986 → **7596 字符**）。**⑥ 边界与未做**：未改任何统计口径 / 交易规则 / 编排语义；未动 Strategy Core / Parameter Search / Backtest / OOS / WFA；`shared/**` 与 `client/**` **零改动**（**未**新增「留档失败」的前端提示 —— 见后续项）；`keepAliveInitialDelay` 保留为可复现的实验开关（`DB_KEEPALIVE_INITIAL_DELAY_MS=30000` 可复现上述崩溃）；🔴 **两条后续项**：(i) 结构级 —— 长算期间**不要跨窗持有连接**（或在写库前才借连接），本轮**未做**；(ii) **待验证假设** —— 保活设有限值 + 给每条约会 `pool.on('connection')` 挂 error 兜底（防 `once('error')` 的 unhandled），可把半开失败的 **19.28 s** 降到 **~0ms**，但**尚未做 A/B，不得当成结论**；⚠️ **验证路径（事后补正）**：本轮**不用 dev server 验证** —— 它其实**一直在跑**（`pnpm run dev` → `tsx watch server/_core/index.ts`，LISTEN `:4000`、`GET /` HTTP 200）；本轮验证一律走**进程内** `appRouter.createCaller`（与页面同一段生产代码，只差传输层）⇒ 改 `server/**`（含注释）**触发了它两次热重启**，已按纪律补跑在途 Run 探针 = **无真实在途**（`closed_loop_backtest_run` RUNNING = 0；唯一 RUNNING 行是 **09-17 的僵死** `archive_research_question`）。
...
> ⚠️ **2026-09-21 补正（`9cm` 收尾②，`BD-21` 真库体检顺带实查）**：下表为 **2026-09-12** 的历史快照（当时为 0 行，是验证脚本逆序清理后的基线）。**当前实查** = `strategies` **12** 行 / `strategy_versions` **13** 行（来源 `docs/evidence/_probe_bd21_recipe_fallback_real_db.out.json`，只读）。本节其余数据域快照**未重查**（本轮不重跑全量数据审计）。
>
...
> 🔴 **编号台账（禁「末条 +1」）**：编号已用至 **`9co`**（`9co` = COMBO-BT-EXP-001，2026-09-24 登记）⇒ **下一个未占用 = `9cp`**。已完成的 `1~9` 与 `9a~9bh`、以及第 10~27 项中的 ✅ 已完成项，已**全部移入 `ROADMAP-CHANGELOG.md`**（原样搬运）。本项目历来按「**下一个未占用**」取号，**禁按「末条 +1」推算**。
> 检索已归档条目：`grep "9a" ROADMAP-CHANGELOG.md` 或直接 grep 任务 ID。

- **9co. （COMBO-BT-EXP-001 · 组合回测「原始策略」五因子独立测试 · 审计 + 方案定稿）** ⬜ **待开工（2026-09-24 登记）**：承接用户「把组合回测中的原始策略先放进独立实验中进行研究分析」。**① 审计（零代码改动，直连库实测）**：`docs/research/AUDIT-COMBO-BT-EXP-001.md` —— 🔴 **先纠正一处命名误导**：实验 `combo-backtest/leader-candidate-baseline` 实现的其实是 `/backtest` 五策略面板的 `baseline`（`server/downsideRisk.ts:785`，label 字面就是「原始策略」，跑 research-legacy 模拟器 `simulateRealisticTPlus1ToTPlus2`），而 `leader-candidate-baseline` 在服务端是**另一个**生产 Strategy Engine 策略（hold-while-selected，有 `engineNonEquivalence.test.ts` 钉死非等价）⇒ **名字指向了错的对象**。现有实验属**重写非复刻**，逐行核实 **8 处口径差异**（候选准入两层 + 数据校准阈值 vs 固定 65、连板数交易日历 vs 事件日期集、滑点分层 vs 固定 10bps、最低佣金有无、分仓分母先后、停牌复牌、`MAX_HOLDING_DAYS` schema 自述 30 实为 5、`parameters: []`）；数据集 `combo-v1`（`dataset_version.id=690001` / READY / **16,221** 事件 / 2025-11..2026-09 / 事件定义 `limitUp` 正确）**仅 11 个月单区间且 `excludeSt:false`**；合规缺口 = 已声明 `FULL_DATASET` 却未出 `unscannedEventCount`。**② 方案（已批准 · 🔴 2026-09-24 23:50 按裁定 5 重写）**：`docs/research/PLAN-COMBO-BT-EXP-001.md` —— 研究口径由「策略镜像 + 综合评分敏感性」**变更为「每个因子单独测试」** ⇒ 建**五个独立实验**（连板数 / 题材家数 / 封板时间 / 成交额 / 流通市值），各走「**分桶 + 日期聚类 Moving Block Bootstrap**」范式（**照抄** `first-board-pullback/*-study` 的既有做法，桶边界直接取服务端评分档位），产出**每个因子的有效区间**；**不做**综合评分（权重敏感性 / 消融 / 因子合成）、**不做**策略回测。分期：Phase A 共享基座 `research-experiments/shared/comboFactor/**` → Phase B（F3 封板时间 / F4 成交额 / F5 流通市值，直接取列）→ Phase C（F1 连板数 / F2 题材家数，需派生）→ Phase D 跨因子对比 + 报告。**废止**旧版的 Phase 0（修 8 处口径 + 逐笔对拍）、Phase 1（参数化）、R2 / R4 / R5（阈值 / 退出 / 分仓）—— 单因子分桶研究不涉及策略语义；R3「单因子消融」**被本方案取代**（消融仍属综合评分框架）；R6 保留为**可选后续**，不排期。**③ 用户四项裁定（2026-09-24）**：基调 = **A 严格对齐服务端**；改名 = **保留 id 改展示名**（目录名 / `descriptor.id` / `manifest.ts` / `pages.ts` key 一律不动，避免影响已持久化 Run）；样本 = **排除 ST + 北交所 + 创业板 + 科创板**；**不建 `combo-v2`**（2026-09-24 23:24 用户更正，上一轮误记为「建」）。**④ 由裁定派生的三条硬事实（⚠️ 2026-09-24 23:50 起：对拍已移出本次范围 ⇒ 下列 **D1 / D2 留档备查**，D3 仍适用）**：**D1** 服务端 `isMainBoardStock`（`leaderCandidates.ts:493`、`db.ts:2049`）= `!/^(300|301|688|920)/` **既不排除 ST、也不排除 `43/83/87/88/4/8` 开头的北交所** ⇒ 「默认对齐服务端」与「默认全排除」**不能同时成立** ⇒ 样本口径做成双档参数（`universeRule`: `server-legacy` 默认 / `strict-main` 研究档 + `excludeSt` 默认 `false`）。**D2** Dataset 侧 `main` = `classifyBoard`（`server/data/boardRules.ts:76`）= `60/000/001/002/003`，**是服务端放行集的真子集**，差集（北交所 `43/83/87/88/4/8` + `689`）**在数据集中根本不存在** ⇒ **Phase 0 对拍作用域 = 沪深主板（含 ST），`S\D` 不可覆盖，禁止把「作用域内逐笔全等」表述为「完全等价」**。**D3** 不建长窗口 ⇒ 研究窗口锁定 `combo-v1`（**11 个月** / ≈3.7 季度）⇒ **结论不可外推**；原「两窗口同号才写进报告」判据**作废**，替代 = **日期聚类 Bootstrap 95% CI + R6 随机基准对照 + 强制区间限定**；⚠️ `v5` **不能当替代窗口**（`firstBoard` 事件定义 ⇒ 评分里的 `boards` 恒为 1、高位连板风控失效 ⇒ **不是同一个策略**）；R7 降级为窗口内季度切片（仅 3~4 段，**描述性、不得宣称稳健**）。✅ `boards:["main"]` 由 `classifyBoard` 天然覆盖裁定 3 的创业板 / 科创板 / 北交所三项 ⇒ **将来若条件触发**要建 `combo-v2` 时**无需改构建器**，只需显式 `excludeSt: true`（留档备查，本轮不执行）。**⑤ 裁定 5 变更后的新增硬事实（2026-09-24 23:30~23:50 只读直查；详见审计 §9 与方案 §0.1 D1~D4）**：**D1** 🔴 **命名撞车** —— dataset 的 `turnover` 是**换手率（%）**，而**成交额（亿元）在 `sourceTurnoverAmount`**、**流通市值（亿元）在 `sourceCirculationValue`**（服务端评分档位对应的正是后两者）⇒ **取错列研究即报废**。**D2** 「有效区间」被定义为**三态统计判定**（日期聚类 Bootstrap CI **下界 > 0** = 有效·正向 / **上界 < 0** = 有效·负向 / 跨 0 = 不确定）+ 样本 **< 100** 强制判「样本不足」；🔴 **禁**按中位数最高挑「最优区间」（`research-experiments/template/README.md:85` 明令「不在实验里宣称最优/最佳/排名」）；且须披露**多重比较**（5 因子 × ~6 桶 × 3 视界 ≈ **90 个判定**，α=0.05 下假阳性期望 **≈4.5 个** ⇒ 声明为**探索性** + Bonferroni 校正）。**D3** 🔴 **排除 ST 在实验内做不到** —— dataset 事件表 **27 列无任何 ST 信号**（`excludeSt:false`；`research_security_status_history` 的 `statusType='ST'` **841** 行属**服务端表**，实验读不到；`limit_up_records.stockName LIKE 'ST%'` 窗口内仅 **1** 行）⇒ **待用户裁定**（方案 §10 建议：选项 A 接受不排除 + 登记 `LIMITATION`）。**D4** 🔴 因子列（`sector` / `limitUpTime` / `sourceTurnoverAmount` / `sourceCirculationValue`）**同批缺失**、填充率仅 **82.4%**（13,3xx / 16,221）且**单调偏向早期**（2025-11 缺 22.5% → 2026-08 缺 4.1% → 2026-09 缺 1.2%）⇒ 可用样本存在**时间选择偏差**，须逐月披露并做缺失期对照切片。✅ 裁定 3 的**北交所 / 创业板 / 科创板已天然满足**（实测 `boardType` **100% = `main`**，`chinext`/`star`/`bse` 零条）；✅ 题材归一化 `normalizeSectorName` / `resolveThemeWithFallback` 都在 **`shared/`** 且**现有实验已有 import 先例**（无合规障碍）；⚠️ 但实验读的是已过滤 `main` 的事件表 ⇒ 算出的「同题材家数」**系统性小于服务端全市场口径**，须显式声明并可做口径对照。🔴 `marketCap` / `floatMarketCap` **0% 填充**（完全不可用）；🔴 `limit_up_records.boardCount` 取值分布不合理（`"1"` 占 10,731、`"2"` 仅 1、`"3"/"4"` 为 0 却有一大簇 ≥5）⇒ **不可作连板数真源，必须派生**。**状态**：审计 + 方案**已完成（本轮零代码改动）**，Phase A 起为代码实施，**待用户下达开工指令**（并先裁定 D3 的 ST 处置）。

- **⏭️ NEXT（用户规格 §十 · 2026-09-21 登记）：`Parameter Search — Actual Research Run`** —— `STRATEGY-RESEARCH-BRIDGE-001` 收尾（`BD-21`）已于同日完成，**不新建**任何 Closeout / Audit 任务；下一步 = 在 `first-board-pullback@1.0.0` × `Dataset 390002` 上**继续真实运行**参数搜索（第一轮 smoke 4 组合 + 第二轮 expand 9 组合**已 PASS**，后续扩大搜索规模 / 换窗口 / 进入稳健性与 OOS）。⚠️ 只有**真实运行中发现的明确阻塞问题**才允许再开最小修复（定位根因 → 最小修复 → 重新运行 → 继续）。

- **⏸️ PLANNED（方案 B · 2026-09-21 用户裁定 · **暂不执行**）：独立实验 Dataset Provider 化 / 多 Dataset 扩展** —— **架构选择 = 方案 B**：后续不按 `datasetCode` 在独立实验层增加分支，也不另写第二套取数核心；把现有 `DatasetPlugin` 从「构建插件」升级为「构建 + Schema + Reader」三位一体的 `DatasetProvider`，由 Provider Registry 按 `datasetCode` 提供版本事实、角色 / 字段 / PIT 元数据与有界读取能力，使 `ExperimentDatasetPort` 只依赖 Provider，不再硬编码 `first_limit_pullback` 物理表。**计划分阶段（开工后才执行）**：① 定义 `DatasetSchema / DatasetProvider / DatasetAccess` 契约；② 将现有 `first_limit_pullback` 适配器原样搬入 Provider，保持既有实验结果不变；③ 改造 `ExperimentDatasetPort` 走 Provider Registry；④ 接入第二个 Dataset 证明扩展路径；⑤ 再将实验契约从单个 `datasetRequirement` 扩展为多 `datasetBindings`（主 Dataset + 辅助 Dataset）。**当前状态 = DESIGN / PLANNED，未开工**：只登记架构决策，本轮**零代码 / 零契约 / 零 migration / 零 DB 改动**。触发条件 = 用户明确下达开工指令；开工时按编号台账取下一个未占用号，本条目**不预占编号**。

- **EXPERIMENT-HARDENING-001. （现有独立实验可靠性补强 · 未另占编号）** ✅ **已完成（2026-09-21）**：把独立实验从「同步长请求 + 代码身份不可证 + 未来读取只靠声明」补到「后台有界队列 + 代码指纹 + 样本冻结 + 资源配额」：新增 `researchExperiments.startRun` 与进程内单并发队列（明确不冒充多实例持久队列）；Run 新增 `experimentCodeDigest`（migration `0048`，可空兼容历史行）；Runner / Manifest / Result / 前端详情贯通代码指纹；`observation()` 必须先 `freezeSelection()`，空样本冻结后返回空集而不读取未来行情；Result JSON、表格行列、Artifact 数量 / 单文件 / 总量增加上传前配额，失败时清理本次已写对象；后台生命周期异常兜底收敛为 `FAILED`。**验证**：`tsc --noEmit` 0；`pnpm run build` exit 0；`tests/server/researchExperiments` **16 文件 / 311 例全绿**；真实 DB + MinIO 小样本 Run `RUN-20260921-A0E433D0` 在 34027ms 后 `COMPLETED`，Result / Manifest / 2 个产物可读；`test:changed` 的失败文件全部落在既有环境依赖基线，**零新增失败文件**；migration dry-run `pass=true / skippedExisting=1`；EOL 漂移 0。**未做**：跨 Run 比较、多实例持久任务系统、方案 B Dataset Provider。详见 `docs/architecture/CHANGE-AUDIT.md` 同名节。

- **EXPERIMENT-DECISION-FORWARD-001. （`first-board-pullback/decision-forward-study` · 未另占编号）** ✅ **实验已建立，待首次真实运行**：新增决策时点后续收益实验，严格把收益窗口限制为 `rd ∈ [k+1, h]`，避免沿用 EXP-001「事件日锚定 + 路径与收益重叠」口径。默认参数 = 决策日 `[1,2,3,4,5]` × 视界 `[10,20]`、`maxEvents=400000`、往返成本 `20bps`；输出不破 / 破位、回撤深度分桶、平均净收益 95% 近似 CI、分年度结果，以及逐决策日 / 视界比较；不输出最优日、最优视界、策略或候选。**验证**：`tsc --noEmit` 0；`pnpm run build` exit 0；`tests/server/researchExperiments` **17 文件 / 315 例全绿**（含 `decisionForwardStudy.test.ts` 3 例与 `manifest.test.ts` 9 例）。**下一步**：通过 `/research-experiments/first-board-pullback/decision-forward-study` 使用 Dataset `390002` 启动真实 Run；本轮未执行真实 Run，也未创建策略。

- **RESEARCH-PROTOCOL-PROVIDER-GATE-001. （Research Protocol / Dataset Provider / Confirmatory Gate · 未另占编号）** ✅ **P0 架构三项已完成（2026-09-21）**：① **Research Protocol**：新增 `EXPLORATORY / OBSERVATION / HOLDOUT` 阶段、协议指纹、Observation / Holdout 窗口、`parentRunId` 与单次 Holdout 锁；Dataset Port 按评估窗口真实过滤事件；② **Dataset Provider**：新增 `ExperimentDatasetProviderRegistry`，生产装配不再直接绑死 `first_limit_pullback`，并支持 primary + auxiliary Dataset bindings 与 `context.datasets`；③ **Confirmatory Gate**：Run 持久化 Gate，OBSERVATION 只能产出 `OBSERVATION_READY / INSUFFICIENT`，HOLDOUT 只能产出 `PASS / FAIL / INSUFFICIENT`；Strategy Bridge 只接受 `HOLDOUT + PASS + 同协议指纹` 的证据。**验证**：`tsc --noEmit` 0；`pnpm run build` exit 0；`tests/server/researchExperiments` **19 文件 / 322 例全绿**；migration `0049` 已真实应用并幂等复跑 `11 skipped / pass=true`。**边界**：当前生产只注册 `first_limit_pullback` provider；指数 / 行业 / Market Regime 等第二数据源仍需逐项实现 Provider 与不可变版本。

- **RESEARCH-PROTOCOL-HOLDOUT-ISOLATION-001. （确认性 Holdout 数据隔离 + 首板回踩确认性运行 · 未另占编号）** ✅ **已完成（2026-09-22）**：用现有 `first-board-pullback/decision-forward-study` 真实执行 Observation → Holdout，并修复运行中暴露的“Holdout 窗口已被历史探索 Run 看过”缺口。**真实结果**：Observation `RUN-20260921-8AF91CE9`（`OBSERVATION_READY`，15,297 样本，H1 预检 **0/10**）；Holdout `RUN-20260921-54D654BB`（机械化 Gate `PASS`，8,415 样本，H1 **8/10**）。🔴 **结论不是“确认成功”**：2026 Holdout 已被探索 Run `RUN-20260921-A87E7438`（全 Dataset 窗口）看过 ⇒ `_probe_holdout_contamination.mts` 判 `contaminated=true`，不能作为干净 OOS，禁止创建正式策略。**新门禁**：`EXPERIMENT_PROTOCOL_HOLDOUT_CONTAMINATED` 在启动前拒绝重叠窗口；Strategy Bridge 再次审计并拒绝已污染证据；历史未声明窗口按 Dataset 全窗处理。另修复 Run DATE 读回为 `YYYY-MM-DD`。**验证**：`tsc --noEmit` 0；`pnpm run build` exit 0；`tests/server/researchExperiments` **19 文件 / 327 例全绿**；真实复跑 Holdout 被污染门禁拒绝；详情读回 `CONTAMINATED`。**下一步**：不要继续换决策日 / 视界找有利格子；要补按日期聚类 Bootstrap、真实执行模型与真正未看过的未来 Holdout，或终止该方向。

- **LIMIT-EXECUTION-FACTS-001. （主板执行事实层 + 2019–2024H1 确认性 Dataset v3 · 未另占编号）** ✅ **已完成（2026-09-22）**：按冻结方案补齐 `prevClose / limitUpPrice / limitDownPrice / suspension / tradability`，先只做沪深主板。**实现**：migration `0050`（event 4 列 + prefix 1 列 + post 15 列）；首板与执行事实全部改为交易日感知涨跌停规则（创业板 2020-08-24 前 10% / 后 20%）；停牌取 PIT `TRADING / SUSPENSION`，缺 bar 不冒充停牌；一字涨停不可买、一字跌停不可卖；`preClose` 直接使用交易所前收并按分价四舍五入计算涨跌停价。**真实构建**：`first_limit_pullback@v3-confirmatory`（`dataset_version.id=540002`）`2019-01-01..2024-08-31`、`main + excludeSt`、T-20..T+20、horizons `[5,10,20]` ⇒ **49,154 events / 3,189,359 rows / READY**。Observation `2019–2021` **26,383** 事件，Holdout `2022–2024H1` **22,771** 事件；post **983,080** 行，其中前收非空 **979,495**、停牌 **2,717**、一字涨停 **6,807**、一字跌停 **2,259**、可买 **967,332**、可卖 **959,733**。**验证**：`tsc --noEmit` 0；`tests/server/datasetRegistry` **13 文件 / 205 例全绿**；migration 幂等复跑 `20 skipped / pass=true`；真库验收探针 `_probe_v3_execution_facts.mts`。**边界**：只覆盖主板；指数 / 行业 / Regime 辅助 Provider 尚未实现；日线级可交易性不等于逐笔执行模型。

- **HOLD-OPEN-PRICE-PULLBACK-001. （首板后回撤但不破开盘价交易研究 · 未另占编号）** ✅ **已完成首轮独立实验（2026-09-22）**：新增 `first-board-pullback/hold-open-price-pullback`，默认口径 = 首板后 T+1..T+5 等待首次相对首板收盘回撤 `100bps`，等待路径不得跌破首板开盘价；触发后次一交易日开盘入场，T+10/T+15/T+20 收盘固定退出。**真实 Run** `RUN-20260921-35357C36`（Dataset `540002`，`COMPLETED / SUCCEEDED`，234,032ms）：候选 **49,154**、eligible **48,897**；入场前破位 **5,221**、未触发 **9,039**、触发 **34,637**、触发后不可买 **143**、交易样本 **101,751**。交易模式 T+10/T+15/T+20 中位净收益分别 **-1.80% / -2.83% / -3.57%**，但均优于全部样本 T+1 开盘入场基准（基准分别 **-3.23% / -4.15% / -4.92%**）；尚未做日期聚类 Bootstrap 或 Holdout 确认。**验证**：`tsc --noEmit` 0；新增实验测试 + 实验模块全绿；结果 / Manifest / 日志 3 个对象可读。**边界**：入场后破位只登记事实，未实现破位后实际退出；v3 只覆盖沪深主板。

- **HOLD-OPEN-PRICE-PULLBACK-002. （一字板首板排除复跑 · 未另占编号）** ✅ **已完成（2026-09-22）**：实验 `1.1.0` 新增 `excludeOneWordLimitUp`，首板日 `O=H=L=C=limitUpPrice` 视为一字板并默认排除。真实复跑 `RUN-20260922-2090E5A6`（Dataset `540002`，`COMPLETED / SUCCEEDED`，777,706ms）：识别一字板 **1,621** 个并全部剔除；eligible **48,897 → 47,276**，入场前破位 **5,221 → 4,214**，未触发 **9,039 → 8,425**。触发 **34,637**、触发后不可买 **143**、交易样本 **101,751** 以及 T+10/T+15/T+20 全部收益指标均**逐位不变** ⇒ 一字板事件原先就没有进入可交易触发集，本次排除对该模式结果没有增量影响。

- **FIRST-BOARD-CONTEXT-BOOTSTRAP-001. （首板前上下文 / 首板后振幅 / 换手率 + 日期聚类 Bootstrap · 未另占编号）** ✅ **已完成首轮独立实验（2026-09-22）**：新增 `pre-event-context-study`、`post-event-amplitude-study`、`turnover-study`，并引入共享 Moving Block Bootstrap。**真实结果**：① 首板前间隔与涨幅 `RUN-20260922-71838486`：间隔 1–3 日 T+20 中位净收益 **-9.33%**，>20 日 **-3.61%**；T-10 前期涨幅 <-10% 的 T+20 中位 **+2.15%**，>+20% 为 **-8.87%**；② 无涨跌停/振幅 `RUN-20260922-3F4B3C39`：无涨跌停 25,409 / 有涨跌停 23,488，无涨跌停组三种视界均更好，平均振幅 ≥8% 的 T+20 中位 **-4.61%**；③ 换手率 `RUN-20260922-4A0DAF7B`：换手率非空 49,150、流通市值非空 **0**；≥10% 换手率的日期聚类 Bootstrap 95% CI 在 T+5/T+10/T+20 均完全低于 0。**边界**：流通市值底层 `circ_mv` 全缺失，关系返回 `INSUFFICIENT_DATA`；三项实验均使用完整 2019–2024H1，故不能再把该区间当独立 Holdout，后续需未来未读数据。

- **STRATEGY-EXIT-PARITY-001. （策略退出执行一致性 + 回测留档可见性 + Run 分页 · 2026-09-21）** ✅ **已完成代码与单测**：把策略文档中的 `STOP_LOSS / TAKE_PROFIT / TIME_EXIT` 从“仅声明”推进为真实执行政策。止损 / 止盈按 `INTRADAY` 用当日 OHLC 触发，时间退出按 `ON_CLOSE` 触发、下一交易日开盘成交；`Trade.reason` 贯通止损、止盈、持有期满与候选退出。闭环回测在进入模拟器前按 Dataset 行注入 `securityBoards`，修复主板 / 创业板 / 科创板 / 北交所涨跌停规则未注入的单一路径。`loopRun` 返回 `persistence`，回测历史写入失败时页面显示响亮的红色告警，而不再只有服务端日志。Run 历史增加 `offset` 分页，前端提供上一页 / 下一页。**验证**：`tsc --noEmit` 0；`pnpm run build` exit 0；策略装配 / 模拟器 / 持久化 / 适配器定向测试 **426 例全绿**。**边界**：公司行为复权仍未进入模拟器；队列仍是单进程；`K2` 保持显式缺口。
...
- **9cn. （`BD-24` · 「跑完策略在『回测历史』里看不到」—— 长算后留档写库撞死连接 · 用户报障）** ✅ **已完成（2026-09-21）**：用户原话「刚才我跑了一下first-board-pullback，在回测历史中没有看到」（入口 = 策略页「运行策略」，页面**出了结果**）。**① 定位（不是「没跑」也不是「还在算」）**：「回测历史」唯一数据源 `closed_loop_backtest_run` 全库 **8** 行、最新 **09-19**、`fbpRows = 0`；唯一写入点 = `loopRun → persistClosedLoopBacktestRun`（**best-effort**：全失败只 `console.warn`，故页面照出结果、库缺行）。dev server 已停 ⇒ 改**进程内** `appRouter.createCaller` 复现，抓到原始错误：3 次 INSERT + 紧随的 SELECT **全部** `read ECONNRESET`（errno −4077，DB = `gateway03.us-east-1.prod.aws.tidbcloud.com:4000`）。**② 根因（两条，全部实测）**：池 `idleTimeout` **600s** **大于**实测链路空闲窗口 **∈ (240, 330] s** ⇒ 588,881 ms 的长算一条都没被回收；且 `keepAliveInitialDelay: 0` ⇒ 保活在分钟级尺度上等于没开。**③ 修复**：`resolveIdleTimeoutMs()` 默认 **180_000**（判据 = 严格小于实测窗口下界 240s，导出常量 + 可证伪单测交叉核对落盘证据的 `aliveAtSec` 最大值）+ 留档重试 **3 → 6**（派生自「池内实测至少 4 条已死」）且**只重试瞬时错误**。**④ 能力边界（实测纠正）**：回收器只处理**自由队列**且只看 `release()` 刷新的 `lastActiveTime` ⇒ 「借出跨越掐断窗口」的连接**永久不可见**（源码 `pool.js:198-210` / `pool_connection.js:23-30`）⇒ 修后陈旧连接 **≥4 → 1**（生效）**但第 1 次尝试仍失败**（19.28 s 半开超时）、第 2 次换新连接 3.86 s 成功 ⇒ **承重项是重试预算，不是 `idleTimeout`**。**⑤ 一次被证伪的修复**：把保活初始延迟改成 30s ⇒ A/B 探针证伪其假设，且真跑**崩进程**（EPIPE 双 emit vs mysql2 `once('error')`）⇒ 已回退并就地写红线。**⑥ 修前 / 修后真跑对照**：`fbpRows` **0 → 1** ⇒ 「回测历史」出现该行。详见 `docs/evidence/README.md` 的 `BD-24` 节。
- **9cm. （STRATEGY-RESEARCH-BRIDGE-001 · Independent Experiment → Strategy 正式桥接 + 首板回踩 Strategy 首条真实闭环 · 用户规格 25 节）** ✅ **已完成（2026-09-21）**：**① 定位**：规格 §0 的执行原则是「**不恢复旧链、不建第二套研究系统、不新增独立 Strategy Core、优先复用**」，而仓库里早已存在 `server/researchExperiments/strategyBridge.ts`（单实验桥）与 `strategy_research_provenance` 表 ⇒ 本轮的活是**把「一条链」做成「可追溯 + 可被下游真实消费」**，不是另起一层。**② 交付（复用而非重建）**：**零新表 / 零 migration / 零新依赖**（migration 已用至 `0047`，**未增** `0048`）；桥**扩展**而非新建（`createStrategyFromEvidenceRuns` 与既有 `createStrategyFromExperiment` 并存）；证据快照进既有 `strategy_research_provenance.sourceSnapshotJson`（JSON 能承载就不建表）。生产面 3 新增 / 3 新增路由与适配 + **2 条真缺陷修复**。**③ 链路真实闭合**：`Dataset Version 390002`（`first_limit_pullback` v2 / READY / 声明 **23978** / 窗口 2024-09-01~2026-09-01）→ **3 个真实 Run**（`RUN-20260921-8557F38A` = EXP-001 `1.1.0`；`RUN-20260921-C95B1D47` = EXP-002 `1.0.0`；`RUN-20260921-A95F5B48` 亦为 EXP-002 但**刻意不进证据集**）→ `first-board-pullback v1.0.0`（`strategy_versions.id = 1110001`、`provenance.id = 630001`）。证据 **5 条**：3 条引 EXP-001（回踩率 / 全量扫描账目缺口 / 决策偏移）、2 条引 EXP-002（总体判定 / 稳定计数）；`resultDigest` 两条**不同**（`exp-sha256:63e77251a95bd…` vs `exp-sha256:6c1156944b343…`）⇒ 真的引了两次不同的运行结果。**④ 证据进「可追溯身份」（规格 §6，本轮唯一一次要求用户裁定的架构分叉）**：现状是 `strategy_versions.fingerprint` = **策略文档**指纹，**看不见**溯源表多了一行 ⇒ 会出现规格明禁的「页面显示有研究来源、fingerprint 不知道变化」。两种改法：⒜ 把证据指纹**混进** `strategy_versions.fingerprint`（会破坏既有「文档未改 ⇒ 指纹不变」语义）；⒝ 证据**独立指纹** + 进溯源身份 + 前端可见 + 测试钉死。**用户裁定 ⒝**（推荐项）⇒ 落地 `provenance.researchEvidenceFingerprint = evi-sha256:f0959a03f21bd16a`，`strategy_versions.fingerprint` 保持 `819c1c50903498e7…` **语义不变**，**零 migration**。两条要求实测成立：重放 ⇒ `created=false`、id 与指纹**逐字节不变**；换一条**可解析但不同**的 `reference` ⇒ **响亮冲突** `EXPERIMENT_STRATEGY_EVIDENCE_PROVENANCE_CONFLICT` 且 **未落任何新行**（`1 → 1`）。**⑤ 研究事实与交易规则严格分离（规格 §3 / §8）**：决策登记实测 **18 条**（任务书原文写 19，按**实测数**写）：`FIXED 16 / TUNABLE 2 / DERIVED 0`，`RESEARCH 2 / DESIGN 16` —— 只有 `entryRule.event` 与 `entryRule.extra.observationWindow` 有研究来源，**其余 16 条逐条列名并明标「该值属于策略设计决策，不来自 EXP-001/EXP-002」**。🔴 **没有把任何研究结论自动翻译成交易规则**；规则写在文档的**声明式条件**里，两条条件的阈值**都是参数引用**（不是硬编码研究数字）⇒ 研究口径被表达成**结构**、阈值留给参数搜索。**⑥ PIT（规格 §11）**：`DECLARED_DECISION_OFFSET_DAYS = 5`，观察窗口 `[1,5]` 交易日，**T+5 为包含式 `rd <= 5`**（与 EXP-001/EXP-002 已验证口径一致 —— 决策日收盘时 `rd=5` 那根 bar **已收出**，属已知信息；写成 `rd < 5` 等于**偷偷把决策时点前移一天**）；窗口上界 **==** 该常量（跨模块钉死，任一端改动即红）。无 look-ahead 的三层证据 = 推导（只用 `rd ∈ [1,k]`）+ **研究侧引用**（第 3 条证据正是 `customPayload.informationBoundary.decisionOffsetDays`）+ 结构级（派生字段全按 `CURRENT_BAR` 求值）。**⑦ 下游真实消费（规格 §14 / §15，本轮靶心）**：⒜ **Parameter Search** —— 经既有 `paramSearch.createSearch` 建 `PSRUN-20260921-ce70772c`（`GRID_SEARCH`，注意**不是** `"grid"`），`searchable` **恰 = 两个 TUNABLE**（`maxBreakDepthRatio,maxVolumeRatio`）、`referenceCheckApplied = true`、`unreferencedTunableCodes = []`、组合数 **44 = 现算网格组合数 44**，固定坐标 `{390002, v2, 2025-06-02~2025-08-29, executionPolicyVersion 1}`；**只建 Search 不跑回测**（`createSearch` 本就不执行），探针**自清** `run 1 / combination 44 / result 0` ⇒ **0 残留**。⒝ **Backtest** —— 经既有 `assembleRunWorkbenchInputs` 真实装配 **13094 ms / 150873 行 / 2967 只证券**，`recipeSource = strategy-declarative-conditions`、`datasetSource = registry`、**`strategyDecisionEngine = strategy-core`**（**不是**旧 `signalEngine` ⇒ §15 的 legacy path **未阻塞、故未修**，符合「只有真正阻塞时才修」）。**⑧ 未跑完整回测（如实登记，规格 §19 允许）**：本轮**没有**取行情快照、**没有**进撮合 —— 理由是**没有任何一条判据依赖回测结果**，而「能被 Backtest 识别」已由装配成功 + 决策源是 Strategy Core 证明；**不把「入口验证」冒名成「结果验证」**。**⑨ 前端可达性（规格 §16）**：`_probe_srb001_strategy_detail_frontend.mjs`（无头 Edge + Node `WebSocket` 直连 CDP 量 DOM）走**完整真实用户路径** = `/strategies` → **点策略卡片**（卡片是**整块 `<button>`**，无「打开」文案）→ 等标签渲染 → **真实鼠标点「版本与状态」**（面板住在这个标签里，详情页**默认标签是「策略定义」**）→ 等**证据区块本体**渲染 = **36 PASS / 0 FAIL × 2 次确定性**；证据区块「研究证据（**5** 条 · 只读）」、指纹徽标与后端**逐字节一致**、3 个中文类别标签（**枚举码不进 DOM**）、3 个 runId、2 个实验编号、5 条 `reference` 路径、数据集坐标 `#390002（v2）`、尾注「不构成本策略的买入 / 卖出规则」全部可见；择优措辞**句子级 + 中英双语 + 排除否定句**扫描 **174 句 / 97 句 offenders 均 0**。**⑩ 两条真实产品缺陷（均已修 + 均以注入式验证证明回归可证伪）**：🔴 ⑴ **「声明式条件」路径对任何参数 code ≠ 已注册配方三参数的策略结构性不可用** —— `compileConditionRecipe` 在**注册期**抛 `RECIPE_PARAMETER_INVALID`（`maxBreakDepthRatio` undefined），根因 = `buildGatesProbe` 把探测参数集**写死**为 `Object.values(PULLBACK_PARAMETER_IDS)`，而合成配方的参数 code 来自**文档** ⇒ `buildGates` 拿到 `undefined`；**修法最小面 3 处**（`recipeRegistry.ts` 的 `gated` 支新增可选 `gateProbeParameterCodes?` + `buildGatesProbe` 补探测集 + `compileConditionRecipe` 下传实际引用的 code），**注入式验证**：把 `probe[name] = 0` 改成 `void name` ⇒ 新增 4 例中 **3 条变红**、失败栈正是 `buildGatesProbe → requireNumericParameter`；还原后 **32 / 32 绿**。⚠️ **为什么 28 例旧单测全绿也没抓到**：原夹具的 `PARAM_SCHEMA` 用了 `max_drawdown` —— **恰好命中**那份硬编码集 ⇒ **缺陷被夹具意外回避**，故新回归的第一条就是**夹具自检**。🔴 ⑵ **溯源仓储读路径缺 `withReadRetry`** —— 前端实测到中间态 `panelState.evidenceBlock = false`（「溯源卡片在、证据不在」）；根因 = `DbStrategyResearchProvenanceRepository` 三个**读**方法未继承全站兜底约定，而前端溯源 query 是 **`retry:false`** ⇒ 服务端**单次**冷启动失败即原样透到页面 ⇒ §16「证据必须可见」落空；修法 = 三个读方法包 `withReadRetry`（**写路径不重试**，重试写会造重复行），**行为回归** `provenanceReadRetry.test.ts` **6 例**（读瞬时失败 ⇒ 调 **2** 次 / 语义错误 ⇒ 只 **1** 次 / 写 ⇒ 只 **1** 次 / 夹具自检 ⇒ **3** 次）；**注入式验证**：摘掉一个读方法的 `withReadRetry` ⇒ **恰好 2 条变红**、另两条仍绿 ⇒ 证明**各自的**重试在生效；还原后 **6 / 6 绿**。**⑪ 本轮登记「判据自身写错 / 工具自身出错」6 条**（前端探针 5 条 + 行尾粗判据 1 条；另有 3 条工具/脚本问题单列）：前端 5 条 = ⒜ 按「打开」文字找入口（实际是**整块 button**）；⒝ 以为面板在默认标签；⒞ 🔴 **Radix Tabs 对 JS `.click()` 返回成功但内容不切换（假 PASS 来源）** ⇒ 必须 CDP `Input.dispatchMouseEvent` 发真实鼠标；⒟ 点标签时机太早；⒟ 等待条件只等**同步 prop**（卡片标题）⇒ 改为等**证据区块本体**；首跑 **25 条假 FAIL**。🔴 **最危险的一条是行尾粗判据**：初版按「已跟踪文件应为 LF」判，报出 **333** 个「不应为 CRLF」，其中含自 **2026-08-30** 起就在磁盘上是 CRLF 的文件（mtime 实证）—— 若不复核，会得出「**本轮引入 329 处漂移**」的**假结论**；修正为「**基线 blob ↔ 工作区**」比对后才成立。**共同病因**：「把**我当时看到的那一行 / 那一种写法**当成契约本身」（与 `9ci` 15 条 / `9cj` 9 条 / `9ck` 2 条 / `9cl` 11 条**同源**）。**⑫ 测试与质量闸**：`tsc --noEmit` **0 error**；定向 `vitest` **28 文件 / 651 例全绿 / exit 0**（含新增 `researchEvidence.test.ts` 23 + `firstBoardPullbackStrategy.test.ts` 14 + `strategyBridgeEvidence.test.ts` 12 + `compile.test.ts` 32 + `provenanceReadRetry.test.ts` 6 + `legacyFreeProductionChain` 7）；🔴 **判据是「零新增失败文件」而不是「全绿」**。**⑬ 在途安全**：动手前跑 `_probe_inflight_runs.mts` —— 活跃表全 **0** 个 `RUNNING`（唯一命中 `archive_research_question:270001` = 2026-09-17 归档表僵死）⇒ 判定可安全改 `server/**`；E2E 自清 parameter_search 3 类行，**策略三张表的行作为交付物刻意保留**。**⑭ ⛔ 环境缺陷（非本轮引入，但阻塞了项目惯用的两道 git 闸）**：项目根目录下任何 `git` 命令均返回 `fatal: not a git repository` —— **根因（git 自身判据，非猜测）**：`is_git_directory()` 要求 `$GIT_DIR` 下同时存在 `HEAD`、`objects/`、**`refs/`**，而本仓 `.git/refs` 与 `.git/packed-refs` **双双缺失**（`objects` / `index` / `logs` 都在，`HEAD` 是 21 字节纯 LF 且内容正确）；**对照实验**：`_scratch` 下新建临时仓库同一 git 二进制**正常** ⇒ 问题在本仓 `.git` 内容。**受损面（对象库盘点）**：剩余 **5792** 对象（blob 3780 / tree 1777 / commit **235**），存活**最新** commit = `07fcbf494032`（**2026-09-19 22:56**），reflog 记录的尖端 `a871b033…`（09-21 02:21 commit / 02:23 push）与 `ORIG_HEAD`（09-20 23:24）**均不可解析**；`index` 1830 个唯一 blob 中 **241 个缺失** —— 🔴 **但这 241 个路径在工作区全部还在** ⇒ **丢的是 git 元数据对象，工作区文件零丢失**。**时间线（mtime）**：pack 2026-09-20 20:50 → 末次 commit/push 09-21 02:21/02:23 → **`.git/index` 15:01:07** → **`.git/objects` 与 `.git/worktrees` 15:02:58**（**早于本任务的行尾闸运行**；本任务改动面零 `.git/**` 写入）。**替代判据**：在 `_scratch` 建**影子 GIT_DIR**（只读，真实 `.git` 零写入）取 `ls-files` / `ls-files --eol` / `cat-file --batch-all-objects` ⇒ ① 基线 blob 双 diff **0 漂移**（457 文件参与比较）；② 本轮 **15 个**改动文件**逐个原始字节** = **15/15 纯 LF**；③ 全仓 `i/lf w/lf` 1243、`i/lf w/crlf` **329**（均 2026-08-30 起长期状态）、`i/crlf w/crlf` 2（约定的 `App.tsx` / `AppShell.tsx`）。🔴 **两层都比原脚本更严**（原脚本是启发式「改动量 ≥20 行且占比过半」，小文件可能漏判）。**修复路径已列（报告 §24.4）但本轮未执行**（落在 `.git` 内部，需显式裁定）。**⑮ 未做（如实登记）**：未跑完整回测（只做装配入口验证）；未做 OOS / Walk-Forward / Paper Trading / Production；未做 EXP-003；未新增 Research Experiment（两个实验目录本轮**零改动**）；未新增 Robustness Engine / Parameter Search 算法 / Backtest Engine；未清理生产库既有孤儿数据；未修 `requireRecipe` 的「无 recipe 时**静默回落**默认配方」路径（登记为 `BD-21`，超 §24 边界）。**⑯ 状态**：`STRATEGY-RESEARCH-BRIDGE-001 = COMPLETE`（§23 判据 **24 / 24**）；报告 = `docs/strategy/STRATEGY-RESEARCH-BRIDGE-001-REPORT.md`（本任务**唯一**最终报告）；E2E = `docs/evidence/_e2e_srb001_bridge_consumption.mts`（**43 / 43 PASS**）；前端探针 = `docs/evidence/_probe_srb001_strategy_detail_frontend.mjs`（**36 / 36 × 2**）；根因探针 = `docs/evidence/_probe_srb001_doc_params.mts`；环境取证 = `docs/evidence/_probe_srb001_git_and_eol_state.py`；新增技术债 `BD-21` / `BD-22`（git 元数据受损）/ `BD-23`（`client/**` 329 个 CRLF 长期状态）；**编号台账两处同步 `9cl` → `9cm`（下一个未占用 = `9cn`）**。**⑰ 下一阶段**：**明确指向 `PARAMETER SEARCH`** —— 本轮已把 `first-board-pullback v1.0.0` × `Dataset 390002` 做成参数搜索的**合法上游输入**（真实消费验证通过）⇒ 下一阶段在该坐标上**执行**搜索并评估参数有效性；⚠️ 前置提醒（`9cc` 已登记）：**「参数消费链通 ≠ 有证据」**，开跑前**必先跑 `_probe_9cc_strategy_params.mts`** 确认规则图引用面非空。 **⑱ `9cm` 收尾（2026-09-21 同日；用户裁定「先备份，再最小修复」+「git 修好后一并转回纯 LF」）**：git 元数据**已修复** —— 整份备份 `.git`（36 文件 / 13 973 963 B）→ **纯新增** `.git/refs`、`.git/refs/heads`、`.git/refs/tags` ⇒ git 立即重新识别仓库 → `git fetch origin`（对象 5788 → **12088**；`origin/main` = `a871b033…` 拉回）→ 恢复 `refs/heads/main` = `a871b033…` → **`rm .git/index && git read-tree HEAD`** 重建索引；全程**未**用 `reset --hard` / `checkout -f` / `clean`。⇒ `git fsck` **rc=0 零输出**、`git multi-pack-index verify` **rc=0**、`show-ref` 双 ref 齐备、`HEAD` = 「第一次自动研究」(2026-09-21 02:21:56)。🔴 修复途中还挖出**第二层损坏**：陈旧 `multi-pack-index` 与 09-20 20:50 被改写的 pack 不匹配 ⇒ `fsck` **rc=32** 报 26 条 pack entry 失败，而 `verify-pack` 判旧 pack **自身完整**、这 26 个 oid **不被任何 ref 引用** ⇒ 备份删除 + `multi-pack-index write` 重建后归零。**329** 个 `i/lf w/crlf` **已全部转回纯 LF**（先 zip 备份 877 167 B；逐文件校验纯 CRLF；`i/lf w/lf` 1478 → **1807**；合计 −80 177 字节）⇒ `checkEolDrift.mjs` **现可运行且报 0 漂移 / 0 新 CRLF**；范围外有意 CRLF 的 5 个不动。🔴 **归一化零内容漂移硬证据**：`git diff --stat HEAD` 前后**逐字相同** = `38 files changed, 3950 insertions(+), 433 deletions(-)（⚠️ 该 `38 / +3950 / −433` 是**归一化完成瞬间**的读数 —— 与归一化**之前**逐字相同，**这才是判据：EOL 归一化没有改变既有改动集**。此后本轮又追加了 `scripts/checkEolDrift.mjs` 注释修正与四处台账登记，而这些**都是被跟踪文件** ⇒ `diff --stat HEAD` 会自然变大；**当前快照一律以 `docs/evidence/_probe_srb001_git_repair_and_eol.out.json` 的 `D_diffStatHead_lastLine` 为准**。）`；`status` 前后同为 **58**；**0 个 `D`** ⇒ 工作区零丢失。⚠️ 途中修掉一个「判据分叉」陷阱：归一化后 `git status` 一度假报 **387**（`git diff --numstat` 仍 38）—— 根因 = 索引条目携带**畸形 stat**（记录 size = 旧 CRLF 尺寸、blob 为 LF 尺寸），且 **`git reset --mixed` 会复用该畸形 stat（无效！）**；正解 = `rm .git/index && git read-tree HEAD`（只动索引）⇒ 索引 stat 修正、`status` 回 58。⇒ 纪律：**「stat 判据」与「内容判据」会分叉，矛盾时拿 `git hash-object` 一锤定音**。`BD-22` / `BD-23` **均已关闭**；证据 = `docs/evidence/_probe_srb001_git_repair_and_eol.py` → `.out.json`。 **⑲ `9cm` 收尾②（2026-09-21 · `BD-21` 闭环 + Parameter Search 首次实际研究运行）**：收尾项 `BD-21`（`requireRecipe` 路径 3 **静默回落**默认配方却与 `explicit-request` **共用同一来源值**）**已闭环** —— `RecipeResolutionSource` 增第 4 值 **`default-fallback`** + 回落点 `console.warn` 留痕 + zod 闭集 / 四处注释 / 前端提示同步，**零新表 / 零 migration / 零新依赖**；真库只读体检 **5 / 5 PASS**（全库 **13** 版本 ⇒ `strategy-document` 9 / `strategy-declarative-conditions` 2 / **`explicit-request` 0** / `default-fallback` **2** ⇒ **修复前 100% 的 `explicit-request` 都是假的**）；新增单测 **4 / 4 PASS** + **注入式可证伪**（改回 `explicit-request` ⇒ **恰好 2 条变红**）；**Parameter Search 首次真实运行** = 两轮各 **11 / 11 PASS**（`PSRUN-20260921-45da2fb2` 4 组合 / `PSRUN-20260921-b43a5716` 9 组合 / **9 / 9 `SUCCEEDED`** / 总收益**全负** / `maxVolumeRatio` **单调敏感** / `maxBreakDepthRatio` 被缩量门槛掩盖）⇒ 按用户规格**不新建**任何 `STRATEGY-BRIDGE-CLOSEOUT-001` / `PARAMETER-AUDIT-xxx` / `ROBUSTNESS-AUDIT-xxx` / `OOS-AUDIT-xxx` / `WFA-AUDIT-xxx`；报告 §26 已追加（75036 B / 纯 LF）。
- **9cl. （EXP-002 — Research Robustness / Stability Validation 完整闭环 · 用户规格 26 节）** ✅ **已完成（2026-09-21）**：把 `9ck` 审计结论 **B** 落地为一次**跨阶段 Robustness 复用**，目标不是再造 Robustness 模块，而是证明**现有 Robustness 核心方法已成为 Research Experiment 的稳定性验证能力**。**① 复用而非重建**：**零新引擎 / 零新表 / 零 migration**；`ResearchRobustnessEngine` / `…Evaluator` / `…Drift` 一个都没有被复制；唯一入口 `research-experiments/robustnessBridge.ts`（92 行 / 零实现 / 零状态 / 零 IO，页面只许引类型，由 `manifest.test.ts` 的 **AST 级闸门**钉住）。真正的交付物 = **泛化层 3 文件 / 1540 行**：`server/research/robustness/dimension.ts`(502) 不透明维度 + 两态指标 + 样本账 / `comparison.ts`(333) 声明式比较 + **唯一**容差实现 / `multiDimension.ts`(705) 多维矩阵 + 序列化 + 确定性指纹。**② 最小泛化**：维度从「4 个策略执行轴的封闭枚举」泛化为**不透明泛型**并**保留四轴兼容**；指标从「恰 3 个交易标量」改为 `Record<string, number>`（词表由调用方声明、核心零白名单）；🔴 **「不可用」升为独立一等状态**（`{ metrics, unavailable }` 必须**恰好覆盖**声明词表）⇒ **「不得伪造 0」从纪律变成结构上不可能违反**。**③ 多维矩阵**：**一个 Run 内** 1 基准 + 16 变体 = **17 行**（`observationDay` 5 / `evaluationHorizon` 3 / `sampleCondition` 8），**共享同一基准**；🔴 明确**不做**「一个维度一个独立 Run 再人工拼矩阵」（N 个 Run 各自产生基准 ⇒ 互不可比）。**④ 真重算四条证据**（规格 §12 禁读 EXP-001 `result.json` 改 label）：⒜ 条件划分恒等式 `15306+8114=23420` 与 `3278+2277+5319+4669+2302+5575=23420`（复制旧结果满足不了）；⒝ 窗口上界单调性 `T5 23712 ≥ 基准 23420 ≥ T20 22777`；⒞ 自检行（`OBS_DAY_T5` / `HORIZON_T10`）delta **恰为 0**；⒟ **对照反证** 非 0 delta 行 **39/48**、超容差 **0** 与 `sensitive` **0** 双向一致、最大 `|Δ|/容差 = 0.7490`、四态 **16/16** 完备。**⑤ 结论（如实、不择优）**：16 个非基准变体 **0 敏感 / 15 稳定 / 1 指标不足 / 0 失败**，总体 **`insufficient`**（因 1 个变体**按定义不可用**，不是因为敏感）；`stable` = 「**未观察到**超容差变化」≠「已证明稳定」。**⑥ 样本账**：核心四桶（可加划分，`missing > invalid > excluded > valid`）基准实测 `23978 = 23420 valid + 0 excluded + 356 missing + 202 invalid`，**17 变体逐个不平项 0**；🔴 信封两式**不同义**、换算显式（`信封 eligible = 核心 valid = 23420`、`信封 excluded = candidate − valid = 558 = 0+356+202`）。**⑦ 真实运行**：Run **`RUN-20260921-46934548`**（`COMPLETED`/`SUCCEEDED`，`durationMs 128475`）；Dataset `first_limit_pullback` **390002 未换**；MinIO **8 对象**（result.json 129592 B / 3 CSV 48·17·17 行 / SVG 14430 B 可闭合 / `robustness-run.json` 54679 B 指纹 `ddd458d62578b483…`）；全量扫描 `unscannedEventCount 0`、实验侧分页 12 轮、硬上限 400000 保留；核心记录 `RBM-64E58D4B7AC2D8AC`。**⑧ 本轮 2 条真缺陷**：🔴 平台 —— `manifest.tables = 4`（`robustness-run.json` 被错分进 tables 段）⇒ 修 `runManifest.ts#inferArtifactDescriptor` 角色优先级 + 2 个新单测，E2E 实测 `tables 3 / artifacts 2`；🔴 产品 —— 稳定性矩阵卡片声称的**红色容差参考线 1.0 在页面上根本不存在**（recharts `ifOverflow='discard'` 越域静默 `return null`）；**第一次修法修错了**（`extendDomain` 在本仓是**空操作** ⇒ 线画到绘图区外 `y1 = -32.75`）；**正解** = 显式 `YAxis domain={[0, ratioAxisMax]}`（值域恒含 1）⇒ 实测 `refLineY1 = 8` / `yTicks [0,0.25,0.5,0.75,1]`，并用 **D15 / D15b / D15c** 三条判据同时钉死。**⑨ 新增结构级闸门**：`公共机制 · 12`（剔注释后逐 token 扫 16 个研究侧标识符），**首跑抓出我自己的假阳性**（`BASELINE_` 命中核心自己的错误码 `RB18X_BASELINE_FAILED`）⇒ 修后 48/48，并**注入式自证可证伪**；同时把 `dimension.ts` 一句旧自述从「假」变「真」。**⑩ 登记「判据自身写错」11 条**（E2E 首跑 7 + 前端 2 + 闸门 2），全部登记「为什么错」而非只刷颜色。**⑪ 验证**：`tsc` 0 error / `test:changed` **零新增失败文件** / 定向 `vitest` **6 文件 223 例全绿** / `checkEolDrift` **0/0** / 真机 E2E **29 步全 PASS** / 前端探针 **56/56 PASS** / 中间态 Run 清理 **0 残留**（刻意保留 2 条）。报告 = `docs/research/EXP-002-REPORT.md`（**19 项** / 774 行）。**未做**：显著性 / OOS / 参数搜索 / 交易成本；未创建任何 Strategy / Candidate；**未清理生产库 9 行孤儿数据（只登记）**；未回收 `BD-17`（超 §25 边界）。🔴 **§25 停止条件未触发**（泛化非大规模重构、未影响既有语义）。**`EXP-002 = COMPLETE`。**
- **9ck. （ROBUSTNESS-CROSS-STAGE-001 · 现有 Robustness 跨阶段通用性审计 · 用户规格 12 节）** ✅ **审计完成（2026-09-21）**：**纯只读审计**（零代码改动 / 零 migration / 零写库），产出唯一报告 `docs/research/ROBUSTNESS-CROSS-STAGE-001.md`（535 行 / 17 节）。**① 先纠正一个易错前提**：本仓「Robustness」**不是一个东西，而是三个语义互不相同的并列模块** —— `server/research/robustness/**`（C-18.1 四轴**扰动重估**，加载入式 evaluator，2,024 行）、`server/research/stochasticRobustness/**`（C-18.2 MC/Bootstrap/成交顺序**重估**，2,452 行）、`server/research/searchRobustness/**`（ROBUSTNESS-001，**零重跑**邻域稳定性分析，3,627 行）；三者**无共享基类**（唯一交叉 = C-18.2 借用 C-18.1 的阈值与漂移类型）。🔴 `searchRobustness` 的语义是「**不重算**」，与「Baseline → 改变条件 → **重新计算** → 比较」**执行语义相反**（import 黑名单 `RERUN_MODULES` 用静态测试钉死）⇒ **它对研究阶段不可复用**，只有 C-18.1/C-18.2 是候选。**② 结论 = B（核心可复用，需要轻量适配）**。**不是 A** —— 分层表实测「五缺四」：`Dimension` 封闭为 4 个**策略执行**轴（`types.ts:66`）、`Metrics` 恰 3 个**交易标量**（`types.ts:85-92`）、`Comparison` 阈值是收益/回撤（`types.ts:149-160`）、`Result` 身份是 `strategyId@strategyVersion`（`types.ts:268-269`），且**强制单轴**（`RB18_AXIS_MIXED`）+ **零持久化**（`evaluate.ts:244-257`）。**不是 C** —— `runRobustnessStress` **零 IO、零 drizzle import、零 `Date.now`/`Math.random`**，`evaluator` 是**参数而非内部实现**（`types.ts:104`）⇒ **它不知道也不需要知道「重算」意味着回测还是重跑研究实验**；且 C-18.2 已在借用它的阈值与漂移类型（`stochasticRobustness/types.ts:49`、`run.ts:21`）⇒「核心可脱离 Strategy 独立工作」有**仓内先例**。**③ 最有价值的一条事实（本轮亲验）**：🔴 **核心比较机制从未跨域复用** —— `assessRobustnessSensitivity`（`drift.ts:329`）/ `computeRobustnessDrift`（`drift.ts:156`）/ `computeRobustnessRunFingerprint`（`serialize.ts:68`）在 `robustness/**` 之外（生产代码）**全仓 grep 0 命中**；跨域被复用的**只有「变体生成器」与「阈值常量」**（`overfittingDetection/parameterSensitivity.ts:31-38`、`factorAblation/adapt.ts:36-39`、`walkForwardRouter.ts:71-75`），其中 `parameterSensitivity` 还**自己复制了第二套漂移判定**（`computeDrift:159` / `applyDriftThresholds:173`）⇒ **同一语义仓内两份实现**。「已有复用先例」**只证明生成器可用，没证明判定机制可用** —— 后者恰是 EXP-002 真正需要的那一半。**④ 研究侧现状**：`shared/researchExperimentsContracts.ts` 对 `robust|variant|baseline|sensitivity|stability` **零命中**；`server/researchExperiments/**` 无任何 robustness 能力 ⇒ **研究实验体系内零 Robustness**。🔴 新发现 **P1-1**：`docs/research/EXPERIMENT-CODE-SPEC.md:559`（§M 禁止事项）写「改 Strategy Core / 重做 … / **Robustness** ｜ 本体系不碰这些能力」，**禁的是「改 / 重做模块本身」而非「消费其方法」**，但字面表述会被 EXP-002 作者误读为**过度禁止**。🔴 新发现 **P1-2**：「研究发现稳定性」在**活模块**里是个**只声明、无生产者**的类型 —— `server/research/vocabulary.ts:733` 的 `ResearchFindingStability`（全仓仅 2 处命中，文档注释自述「**按年切片**看效应是否同号、标记 `stable`/`contradicted`」）——**正是 EXP-002 要做的事，只是切片维度不同**；⚠️ 但宿主接口 `ResearchFinding` 属**已退役旧 Research** ⇒ 该字段是**双层孤儿**，**不得**据此认为「已有能力」。**⑤ 真库实测**（新建只读探针 `docs/evidence/_probe_robustness_cross_stage_state.mts`，**每条 SQL 均为 SELECT**，带形状自检 `unwrapped=true`）：全库 `%robust%` 表**恰 3 张**（全是 `search_robustness_*`）；`robustness_run` / `robustness_sample` / `stochastic_robustness_run` **均不存在** ⇒ **C-18.1/C-18.2 零持久化 = 实测事实**（不只是「代码看起来没写」）；`search_robustness_run` **0** 行 / `_result` **6** / `_parameter_analysis` **3**；`parameter_search_run` 共 5 个 Run，`referenceCheckApplied IS NULL` = **3** ⇒ **3/5 历史 Run 处于「参数引用未验证」**。🔴 附带发现 **P2-1**：`search_robustness_result`(6) + `_parameter_analysis`(3) 的 `robustnessRunId` 在父表**找不到对应行**（父表 0 行）⇒ **9 行孤儿**；根因**推断**为 `9bu` 的 E2E「自建自清」只删了父表行（孤儿组数 **3** 与其报告所记「3 个 Robustness Run」吻合，时间戳 `2026-09-20 10:47~10:55` 落在 `9bu` 窗口内），**未直接读清理代码验证** ⇒ 已显式标注为**推断**。**⑥ 最小适配点（仅登记，本轮不执行）**：6 条（轴词表封闭 / 变体配置按轴硬校验 / 指标视图恰 3 标量 / 比较阈值绑定收益回撤 / 强制单轴 + 身份字段 / 零持久化）；其中适配点 6 结论**反向**：🔴 **不要给 C-18.1 补持久化表** —— 研究侧 `research_experiment_run` + MinIO 已投产（`9ch`/`9ci`/`9cj`），补表会造出**第二套 Robustness 持久化**。**⑦ 只读的机器可核证据**：**生产代码 / 契约 / DB 零改动**（`git status --porcelain -- server/ shared/ client/ tests/ drizzle/ scripts/` 与审计开始前**逐行相同**）；⚠️ 台账面按规格 §11 修改了 **4 个文档文件**（`ROADMAP.md` / `ROADMAP-CHANGELOG.md` / `CHANGE-AUDIT.md` / `docs/evidence/README.md`）—— **不是**「零改动」，但**全部落在文档面**；`checkEolDrift.mjs` **0 / 0**；报告 **535 行 / 纯 LF**。**⑧ 对账闸门**（`_scratch/check_robustness_cross_stage_audit.py`）：**75 / 75 PASS**（逐条从真实文件 / 探针 JSON 现读），且**自身可证伪**（篡改探针计数必变红）；🔴 首跑 **2 条 FAIL 全部是判据自身写错**并已登记原因（① 把区间 `:72-82` 当点、写死行号 74；② grep 正则 `(\.\./)+vocabulary` 只认多级相对路径，漏 `./vocabulary` 与 `../research/vocabulary` ⇒ 数出 5（真值 9））—— **同一病因：把「我当时看到的那一行 / 那几种写法」当成契约本身**。**⑨ 边界（如实登记）**：未创建 / 未实现 EXP-002；未改三个 robustness 模块任何一行；未建表 / 未 migration；**未执行 §11 任何适配点**（规格 §8「不为了通用化提前重构」+ §12「先审计事实，再决定架构」）；未验证 C-18.1/C-18.2 在真实数据上的**行为**（它们是「仅测试可达 / 技术预览」）⇒「可复用」是**结构性**判断。**⑩ 状态**：`ROBUSTNESS-CROSS-STAGE-001 = AUDIT_COMPLETE`；**EXP-002 未被标记为开始**。🔴 本轮为只读审计、**未改动项目状态** ⇒ `ROADMAP.md` §44「上轮实查」**保持 `9cj` 不变**（无新状态可覆盖）。
- **9cj. （EXP-001 收口修正 · 全量样本 + 数据质量 + 决策时点条件矩阵）** ✅ **已完成（2026-09-21）**：把 `9ci` 留下的三个口子收掉 —— ⑴ **全量 23978**（`unscannedEventCount = 0`，缺口 3978 → **0**）；⑵ **数据质量两口径**（Bar 次 1651 vs 事件数 334；污染未来收益的坏 Bar = **0**）；⑶ **决策时点矩阵 45 格**（T+1…T+5 × {ALL, NON_BREAK_OPEN, BREAK_OPEN} × T+5/T+10/T+20，窗口**严格在决策时点之后**）。**关键手法**：不重建框架、不删安全阀，而是把「单次扫描 20000 行」变成**合法的分页机制**（`eventScanPolicy: "FULL_DATASET"` 声明式放行 + 硬上限 `400000` 保留 + `eventPages()` 流式分页 + `EXPERIMENT_BAR_BATCH_SIZE = 2000` 分块批量读）；**默认阀 20000 仍是默认值，未声明策略的实验行为完全不变**。真实 Run `RUN-20260921-9D216C34`（Dataset 390002 **未换**、`durationMs = 137638`、ExperimentVersion `1.1.0`），产物 **12 个对象**（6 CSV + 3 SVG）。**八条真缺陷全部已修 + 已加测试钉死**，其中 🔴 ⑹「新增平台能力未同步作者面契约」（规范 / 模板 grep 命中 0 ⇒ 能力对增量等于不存在）与 🔴 ⑺「`droppedByScanLimit` 判据自身写错 ⇒ 假 PASS」最值得记：**判据自身也会错，且错了不会报错，只会安静地给出假结论**。报告 = `docs/research/EXP-001-final.md`（27 节；新增 §22【A】/§23【B】/§24【C】/§25【D】/§26【E】）；E2E = `docs/evidence/_e2e_9ci_exp001.mts`（**25 步全 PASS**）；证据 = `_probe_9cj_preflight.mts` / `_probe_9cj_run_readback.mts` / `_probe_9cj_tables.mts` + `_e2e_9cj_exp001_full_scan.out.json`；前端探针 = `docs/evidence/_probe_9ci_exp001_frontend.mjs`（**46 PASS / 0 FAIL**，新 Run `RUN-20260921-8557F38A`）。🔴 **本轮登记「判据自身写错」9 条**（前端探针 5 + 真机 E2E 1 + 单测 2 + 本轮两个验收闸门 2），全部按项目纪律**登记原因**而非只把颜色刷绿。**停在 EXP-001 COMPLETE；未做稳健性 / OOS / 参数搜索，也未创建任何 Strategy。** 🔴 **规格 §16 完成判据：13 / 13 PASS** —— 闸门 `_scratch/check_9cj_done.py` 逐条**从落盘证据现读**（不读报告怎么写），退出码 0；且**闸门自身可证伪**：注入 `unscannedEventCount = 3978` 后判据 ③ 必然为假（已实测）。⇒ **EXP-001 标记为正式 `COMPLETE`**。

...
> 🔴 铁律：§44 为「覆盖式」状态区（**只保留最近 1 条「上轮实查」**，历史条目在 `ROADMAP-CHANGELOG.md`）；§44.5 为**未完成**队列（编号按「下一个未占用」，已用至 `9ch`）；§47 为 append-only 更新记录。
...
> 上轮实查：**2026-09-20 · RESEARCH-EXPERIMENT-004（独立研究实验持久化 + MinIO 产物存储）（用户规格）**（编号 `9ch`；`tsc --noEmit` **0 error**；`pnpm run build` **exit=0**（vite 15.86s → `dist/public` index-DV7fXQCY.js 2,782.73 kB / index-CehSWSHH.css 236.91 kB + esbuild `dist/index.js` 2.8 MB）；`test:changed` **零新增失败文件**；定向 `vitest run tests/server/researchExperiments tests/server/research/legacyFreeProductionChain.test.ts` = **10 文件 / 141 例全绿**（旧 Research 依赖 Gate 未被 004 破坏）；`checkEolDrift.mjs` 行尾漂移 **0**）。**① 数据库（手工幂等 migration `0047`，零 FK，20 列 / 3 索引）**：新建 `research_experiment_run` 承载 Run 元数据（Run 身份 / 实验坐标快照 / Dataset 坐标 / 参数快照 / 生命周期 / 错误 / `resultManifestKey` / 轻量摘要）；🔴 **刻意不建 `experiment` 表** —— Experiment 元数据是**代码声明**的，再建一张 DB 表会让「代码里的定义」与「DB 里的行」成为两个真源（历史可读性改由**快照列**兑现）；迁移实测 `executed 4 / skipped 0 / pass true`，**复跑** `executed 0 / skipped 4 / pass true` ⇒ 幂等成立；`schema ⇄ DB` 列集 **20 = 20，缺 0 / 多 0**（E2E 第 1 步逐列比对）。**② 对象存储**：新增 `server/artifactStorage/**`（端口 `put/get/exists/delete/list/getMetadata` + 闭集错误码 + `MinIOArtifactStorage`，S3 协议 `forcePathStyle=true`，复用**已装**的 `@aws-sdk/client-s3` ⇒ **零新依赖**）+ `server/researchExperiments/persistence/**`（Run 仓储双实现 / Manifest / 产物装配 / 生命周期编排）；Object Key = `experiments/{group}/{key}/runs/{runId}/{manifest.json|result.json|tables|charts|logs|artifacts}/…`，**结构级防注入**，且**授权判据 = Manifest 白名单**（未登记 Key ⇒ 404、跨 Run Key ⇒ 400，前端真机实测）。**③ 一致性（情况 A/B/C 逐条设防）**：写入顺序 = 建 Run → RUNNING → 执行 → 产 Result → 上传 → 生成 Manifest → **逐条 `exists()` 复核** → 才置 COMPLETED（顺序反过来情况 A 在结构上就成立）；失败一律 `markFailed`，另有 `reconcileRun` 人工收敛**确证卡住**的 RUNNING。**④ API**：tRPC 新增 `listRuns / getRun / getRunResultManifest / getArtifactMetadata / reconcileRun`，`list`/`get`/`run` **扩展而非重建**（并加**反向断言**「同义端点名不存在」，防止把「一个概念两个入口」固化成契约）；产物走 Express **只读**代理 `/api/experiments/artifact`（刻意不走 tRPC：几十 MB 的 Parquet 走 superjson 要 base64 且整块驻留内存）。**⑤ 前端**：列表页新增 **状态 / Latest Run / 最近运行时间** 三列（§15 要求）；详情页新增「运行历史」卡片与「已持久化」回执，并把「**算完了但没能持久化**」与「执行失败」分成绿/红两条提示（§13 的语义可视化）；新路由 `/research-experiments/:group/:key/runs/:runId`。**⑥ 两条真缺陷（都已修 + 已加测试钉死）**：⑴ `NoSuchBucket` 曾被判成 `PUT_FAILED` ⇒ `put()` 里那条「请确认桶已存在」的**自救提示不可达**（正常应拿到一句无信息的「写失败」），修法是把它归入 `UNAVAILABLE`；⑵ Run 的**读**路径缺了全站既有的瞬时读重试兜底 ⇒ dev server 冷启动首次 `listRuns` 真机 **500**（**19.2s 后失败**，与 `server/db.ts` 的 `connectTimeout: 20_000` 同量级；紧接着重试同一请求 1.6s 成功），修法是 5 个读方法挂 `withReadRetry`（**写路径不重试**，重试写可能重复写入），新增 `runRepositoryRetry.test.ts` **7 例**，复测 `list` **19.2s → 0.28s**。**⑦ 验证**：真库 + 真 MinIO + 真 Dataset 的 **12 步最小 E2E 全绿**（含**绕开服务层**直接列举 MinIO 前缀得 **5 个对象**、清理后**残留 0**）；前端 CDP 可达性 **46 PASS / 0 FAIL**（含「**初始化不拉产物 = 0 请求**」「点击后才发产物请求」「未登记 404 / 跨 Run 400」）。**⑧ 旧断言纠正**：001 前端探针里的「**不新增数据库表**」与「本体系**零写库**」两条**已被 004 证伪**，已同步改判据（并把「不要为让它变绿而把旧文案加回产品」写进文件头）。报告 = `docs/research/RESEARCH-EXPERIMENT-004-final.md`；migration = `drizzle/0047_experiment_run_persistence.sql`；E2E = `docs/evidence/_e2e_9ch_experiment_persistence.mts`；前端探针 = `docs/evidence/_probe_9ch_experiment_persistence_frontend.mjs`。**未做（如实登记）**：Parquet / Chart 产物**无真实样本**（链路齐备但未端到端跑过）、跨介质级联删除未实现、`stale` RUNNING 无自动回收定时器、生产链真机 E2E 本轮未重跑。**停在 RESEARCH-EXPERIMENT-004 COMPLETE，未自动开始 EXP-001。**
...
> 🔴 **编号台账（禁「末条 +1」）**：编号已用至 **`9ch`** ⇒ **下一个未占用 = `9ci`**。已完成的 `1~9` 与 `9a~9bh`、以及第 10~27 项中的 ✅ 已完成项，已**全部移入 `ROADMAP-CHANGELOG.md`**（原样搬运）。本项目历来按「**下一个未占用**」取号，**禁按「末条 +1」推算**。

...
describe("closedLoopWiring — 覆盖率探测（取代硬编码 executorBound）", () => {
  ...
  it("摘要如实列出每个未覆盖阶段的 reasonCode 与原因", () => {
    const text = describeClosedLoopWiringCoverage(assessClosedLoopWiringCoverage({}, ["data", "optimization"]));
    expect(text).toContain("0/2");
    expect(text).toContain("executorBound=false");
    expect(text).toContain("data（CL_DATA_NOT_INJECTED）");
    expect(text).toContain("optimization（CL_RUNNER_NOT_INJECTED）");
  });
...
describe("closedLoopWiring — 执行器注册", () => {
  ...
  it("只给 researchDataset → 注册 data 与 regime（regime 仅需 dataset 旁路产物）", () => {
    const runners = createClosedLoopStageRunners({
      researchDataset: datasetFixture() as never,
    });
    expect(Object.keys(runners)).toEqual(["data", "regime"]);
  });
  ...
  it("三件套一次创建：inputs / artifacts / stageRunners 同时返回", () => {
    ...
    expect(Object.keys(wiring.stageRunners)).toEqual(["data", "regime"]);
...
describe("closedLoopWiring — 与编排器集成", () => {
  ...
  it("只注入 data：探针覆盖集合 == 编排器实际执行集合", () => {
    ...
    expect([...blocked].sort()).toEqual([...coverage.uncoveredStages].sort());
    // regime 执行器已注册（仅需 dataset 旁路产物），但 canonical 前驱 overfitting 未装配
    // ⇒ 探测已加前驱门、如实判为未覆盖，与编排器 CL_UPSTREAM_BLOCKED 一致。
    expect(run.request.runnerInjected).toEqual(["data", "regime"]);
    expect(executed).toEqual(["data"]);
    expect(coverage.coveredStages).toEqual(["data"]);
  });

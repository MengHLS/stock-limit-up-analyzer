...
- 🔴 **要让服务「跨调用存活」的正确姿势 = `run_in_background=true` 且命令**末尾禁加 `&`**（`nohup … &` / `disown` / `setsid` / `Start-Process` 在本会话环境全部留不住，跨命令即被回收）。加了 `&` 会让 shell 立即返回、tsx 变孤儿 → 命令结束进程即死，且**日志 0 字节**（假象「没起来」）。正确写法：`node_modules/.bin/tsx watch server/_core/index.ts > _dev.log 2>&1`（无 `&`，由任务系统托管），启动后另起一次调用 `curl --noproxy '*' http://127.0.0.1:3000/` 验 200。

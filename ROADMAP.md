...
> 🔴 铁律：§44 为「覆盖式」状态区（**只保留最近 1 条「上轮实查」**，历史条目在 `ROADMAP-CHANGELOG.md`）；§44.5 为**未完成**队列（编号按「下一个未占用」，已用至 `9br`）；§47 为 append-only 更新记录。
...
> 上轮实查：**2026-09-19 16:12 GMT+8 · BACKTEST-002 收尾二（B-04 收口 / R-02 schema / B-03 真实 Run / R-04 zero-volume）（用户规格）**（编号 `9br`；`tsc` 0 error、`tests/server/backtest` **9 文件 / 105 用例全绿**、`tests/server/strategyCore` **10 文件 / 158 用例全绿**、`tests/server/research/executionConstraints` **1 文件 / 27 用例全绿**、全量 vitest 失败文件集合 **8→8 零新增**（用例 4304→4538）、`vite build` 成功、`checkEolDrift` 0 漂移）。**判定 BACKTEST-002 = COMPLETE**。**消灭「两套口径」的三处差**：① 年化基数 **244→252**（全项目除我之外都是 252）并改用唯一原语 `shared/quant-stats#annualizedReturnFromEquityCurve`（`n = 点数−1`）；② `maxDrawdownPct` **负数→正数幅度**（对齐 `performanceMetrics.analyzeDrawdown.depthPct` / calmar 等全项目既有口径，逐点 `drawdownPct` 仍为有符号）；③ 收益率算式形式统一为 `(end/start−1)×100`（**代数等价但浮点不同** ⇒ 闭环既有测试报 `expected 5.3 to be 5.299999999999994`，这才暴露出来）。**接线**：`canonicalMetrics()` 为唯一实现，闭环 `evaluation` 的 5 个重叠标量改取它（`evaluationRef.metricsSource / canonicalMetrics / annualizationBasis` 新字段；未接线时如实标 `evaluators`）；**R-02** `fixed-amount` 全链进入 Strategy Schema（校验/投影/装配映射/前端 + 真实执行测试）；**B-03 真实 Run 端到端**：`loopRun` 真库跑通 ⇒ 留档 1 行、`resultJson.backtest` 与 `strategyRun` 并列、20 项只读断言全绿（`failures=0`）、**两次独立运行指纹逐字节相同**（确定性）；🔴 **过程中抓出并修掉一个真实缺陷**：长算（593~626 s）后池中连接被静默重置，而留档**只尝试一次** ⇒ **长运行每次都静默丢档**（判据 = 紧随的 SELECT 首发也失败、重试即成功）⇒ `persistClosedLoopBacktestRun` 改**有界重试**（≤3 次，仍 best-effort 不抛）。⚠️ **R-06（新）**：同窗口真实 Run 从 ARCH-002 前的 **14.3 s** 变为 **593~627 s**（≈41×）；实测 Core 单次求值 ≈381 µs ⇒ 只占 **8.7%**，主因**未定位**，且**耗时与请求窗口无关**（1 个月窗口 616 s）⇒ **PARAMETER-001 开工前必须先 profile**。报告 = `docs/research/BACKTEST-002-IMPLEMENTATION-REPORT.md`。
...
> 🔴 **编号台账（禁「末条 +1」）**：编号已用至 **`9br`** ⇒ **下一个未占用 = `9bs`**。已完成的 `1~9` 与 `9a~9bh`、以及第 10~27 项中的 ✅ 已完成项，已**全部移入 `ROADMAP-CHANGELOG.md`**（原样搬运）。本项目历来按「**下一个未占用**」取号，**禁按「末条 +1」推算**。

/**
 * RESEARCH-EXPERIMENT-003 — **策略域** tRPC Router（STEP 15 策略 Schema + 版本化 /
 * STEP 21 生命周期 / Research → Strategy 候选桥）。
 *
 * 改名说明：本文件原为 `server/researchRouter.ts`、命名空间 `research.*`。旧 Research
 * （`researchEngine.*` / `researchPlanner.*`）删除后，剩下的四组端点（`strategy` /
 * `lifecycle` / `metrics` / `strategyCandidate`）**全部是策略域**，与「研究」无关
 * ⇒ 命名空间改为 `strategyDomain.*`，避免在 API 面上留下一套名为 research 的入口。
 *
 * 纪律：
...
// RESEARCH-EXPERIMENT-003 — 研究层 barrel（`server/research/index.ts`）已删除。
// 本 router 只用到两簇符号，改为**显式按文件导入**（barrel 的 `export *` 会把整个
// 旧 Research 目录都变成可达集，等于让「已删除的旧体系」继续留在生产依赖图里）。
import {
  validateStrategyDocument,
  bumpStrategyVersion,
  compareStrategyDocuments,
} from "./research/strategySchema";
import {
...
export const strategyDomainRouter = router({
  /**
   * RESEARCH-006.2 / 006.3 — Research → Strategy 候选桥（`strategyDomain.strategyCandidate.*`）。
   *
...
export type StrategyDomainRouter = typeof strategyDomainRouter;

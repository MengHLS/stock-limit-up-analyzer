...
- 136 行 ｜ 用例声明 10 ｜ describe 1
- 被测源码：`shared/sectorHeatOrder.ts`
- 单跑：`pnpm exec vitest run tests/shared/sectorHeatOrder.test.ts`
- 用例树：
- **题材次序（连板梯队组内 + 热力图行序共用唯一口径）**
  - 主键 = 当日涨停家数降序；当日 0 家仍排在「从未出现」之前
  - 次键 = 窗口内合计降序（只在当日热度相同时才轮到它）
  - 末键 = 题材名升序（当日热度与窗口合计都相同）
  - 选到热力窗口之外的日期 ⇒ 当日热度全 -1，次序退化为「合计 → 名称」，且不丢项
  - 「其他」兜底桶永远压尾 —— 哪怕它当日热度与窗口合计都最大
  - 压尾档内部仍按「当日 → 合计 → 名称」排
  - 梯队的 tieBreak 只在**同一题材内部**生效：不把不同题材的格子交错开
  - 🔴 回归（用户报的「对不上」）：同一热度下，梯队去重后的题材次序 == 热力图行序
  - isTailSector 只认兜底桶本身，不做前缀匹配
  - 同题材同热度视为相等（引擎保持相对次序）

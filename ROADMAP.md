...
> 🔴 铁律：§44 为「覆盖式」状态区（**只保留最近 1 条「上轮实查」**，历史条目在 `ROADMAP-CHANGELOG.md`）；§44.5 为**未完成**队列（编号按「下一个未占用」，已用至 `9cm`）；§47 为 append-only 更新记录。
...
> 上轮实查：**2026-09-21 · `9cm` 收尾②（`BD-21` 闭环 + **Parameter Search 首次实际研究运行**）**（编号 **仍 `9cm`（同号收尾，**未新增编号**）；`git` 元数据同日已修复 ⇒ 本轮**所有闸门都是实跑**（不再是替代判据）；`tsc --noEmit` **0 error**；定向 `vitest` **9 文件 / 167 例全绿 / exit 0**；新增单测 `tests/server/runWorkbenchAssembly/recipeFallback.test.ts` **4 / 4 PASS** + **注入式可证伪**（把兜底改回 `explicit-request` ⇒ **恰好 2 条变红**，另两条仍绿）；真库只读体检 **5 / 5 PASS**；Parameter Search **两轮各 11 / 11 PASS**；报告 `docs/strategy/STRATEGY-RESEARCH-BRIDGE-001-REPORT.md` 追加 **§26**（75036 B / 纯 LF））。
>
> **① 本轮只做一件事（用户规格 §一 / §二 / §十二）**：`STRATEGY-RESEARCH-BRIDGE-001` 的**唯一收尾项 = `BD-21`**，且只修「**真正阻塞 Parameter Search 实际运行**」的问题 —— **不**重审 Strategy / Parameter Search / Robustness / OOS / WFA，**不**为「完整性」重查已完成模块，**不**新建 Closeout / Audit 任务，**不**把非阻塞历史风险扩成新任务。完成后**直接进入** Parameter Search 实际运行。⇒ 原「架构建设 → 审计 → 修复 → 再审计」循环**在这一条停止**。
>
> **② `BD-21`（原问题 → 实际处理 → 验证结果）**：`requireRecipe` 路径 3（文档既无 `recipe`、也无 `definition.entry.conditions`）会落到 `DEFAULT_STRATEGY_RECIPE_ID`（`leader-candidate-baseline` = 按当日涨跌幅取前 5 名），但**来源值被标成 `explicit-request`** —— 与「调用方显式指定 `recipeId`」**共用同一个值** ⇒ 要害**不是「有兜底」，而是「静默换规则」**：产物看起来完全正常、跑的规则却与策略文档无关，参数搜索会把结果记在一个**根本没被执行**的策略定义名下。**最小兼容修法**（**零新表 / 零 migration / 零新依赖**）：⒜ `RecipeResolutionSource` 增第 4 值 **`"default-fallback"`**，路径 3 的**默认常量分支**改用它，`explicit-request` **只**留给「调用方显式传值」；⒝ 回落点**响亮留痕** —— `console.warn`（含策略身份 + 落到的配方 id），刻意放在**唯一判定点**（`requireRecipe` 本身，因为 `assembleStrategySide` 被主入口与 `strategyEvaluation` **两条路径**共用，放调用方**必漏其一**）；⒞ 契约与规范同步：`shared/researchContracts.ts` 的 zod 闭集（`closedLoopRunResultSchema.shape.assembly.shape.recipeSource` —— **不同步 ⇒ 生产上 tRPC `.output()` 拒值**）+ `assemble.ts` 三处注释 + `recipeRegistry.ts` 台账注释 + `docs/research/STEP-A-DECLARATIVE-CONDITIONS-implementation.md` 三路径表；⒟ 前端 `ClosedLoopRunResultPanel.tsx` 兜底时显示琥珀色**「兜底默认 · 文档未声明配方与条件」**（不再只印英文枚举 —— **「静默」的用户可见面就在这里**）。
> 🔴 **真库体检的决定性数字**：全库 **12** 个策略 / **13** 个版本 ⇒ `strategy-document` **9** / `strategy-declarative-conditions` **2** / `explicit-request` **0** / **`default-fallback` 2**（`limit-up-baseline@1.0.0` + `@1.1.0`，各 `warn` **恰好 1** 条）。⇒ **修复前，库里 100% 的 `explicit-request` 都是假的** —— 它们全部是「文档没声明任何可执行内容、装配层自己顶上默认配方」，却被记成「调用方明确要求」。**这不是假想分支：这 2 份文档现在仍走该路径。**
>
> **③ Research → Candidate → Strategy 当前真实闭环状态（复核未变）**：`Dataset Version 390002`（`first_limit_pullback` v2 / READY / 声明 23978 / 窗口 2024-09-01 ~ 2026-09-01）→ EXP-001（`RUN-20260921-8557F38A`）+ EXP-002（`RUN-20260921-C95B1D47`）→ `first-board-pullback@1.0.0`（`strategy_versions.id = 1110001` / `strategy_research_provenance.id = 630001` / `sourceKind = INDEPENDENT_EXPERIMENT` / 证据指纹 `evi-sha256:f0959a03f21bd16a` / 证据 **5** 条）⇒ **链路成立且可追溯**。按规格**不**恢复旧 `Analysis → Finding → Conclusion → Candidate` 链（`sourceCandidateId` / `sourceConclusionId` 仍 **NULL**），**未**新建第二套研究系统。
>
> **④ Strategy → Parameter Search 当前真实连接状态（本轮 = 真实运行，非审计）**：`searchMethod = GRID_SEARCH`；`strategyId/Version = first-board-pullback@1.0.0`；`datasetVersionId = 390002`；TUNABLE = `maxBreakDepthRatio` × `maxVolumeRatio`（`firstBoardPullbackGridCombinationCount()` = 自然网格 **44**）。**§三.B 的链路判据逐条落实**（两轮 `createSearch` 回执本身就是证据）：**版本可读**（`name = 「首板回踩（不破首板日开盘价 + 缩量）」`）；**Dataset 坐标明确且不依赖调用方传值**（`resolvePrimaryDatasetVersionId(document) = 390002`，探针步骤 `0b`）；**tunable 参数可识别**（`searchable` 恰 **2** 个，`fixed = derived = excluded = 0`）；**搜索空间可获取**（`referenceCheckApplied = true` 死参数筛查生效 / `unreferencedTunableCodes = []` 声明为 TUNABLE 的参数**都在** Core 规则图里有引用面 / `parameterSpaceFingerprint = a0f6548a00db8eeb…`）；**既有契约未被绕过** —— 全程走 `appRouter.createCaller` 的**真实 tRPC 端点**（zod 入参 + `.output()` 校验均参与），组合计划与结果**真实落库**（`parameter_search_run` / `_combination` / `_result`）。
>
> | 轮次 | Search Run（真实落库） | 组合 | 真评估 | 缓存复用 | 失败 | 窗口 | 耗时 | 判据 |
> | --- | --- | --- | --- | --- | --- | --- | --- | --- |
> | 第一轮 smoke | `PSRUN-20260921-45da2fb2` | 4 | 4 | 4（**跨 Run** 复用） | 0 | 2025-01-02 ~ 2025-03-31 | 9560 ms | **11 / 11 PASS** |
> | 第二轮 expand | `PSRUN-20260921-b43a5716` | 9 | 9 | 0（**全部真算**） | 0 | 2025-01-02 ~ 2025-06-30 | 105241 ms | **11 / 11 PASS** |
>
> **⑤ 第一轮真实运行结果（规格 §八 的 11 问，摘要；逐组合表见报告 §26.4）**：**参数** = `maxBreakDepthRatio` {0, 0.05, 0.1} × `maxVolumeRatio` {0.5, 1, 1.5} —— 网格取值**不是发明的**（`0 / 0.05 / 0.1` 对齐 EXP-001 的 `drawdownBucketEdgesBps [0, -200, -500, -800, -1000]` bps 桶边界；`0.5 / 1 / 1.5` 是草稿已声明的缩量粗档，`1.0` = 「不超过事件日量」中性边界）；**组合 9**（去重后与之一致 ⇒ 确为笛卡尔积）；**9 / 9 `SUCCEEDED`**、`failedCount = 0`（无失败组合 ⇒ 「失败原因」不适用）；**指标**（交易数 / 总收益% / 最大回撤% / 胜率% / 盈亏比）逐组合落库并可读回（`persistedResultRowCount = 9`，独立核对 `parameter_search_result` 行数 == 组合数）。**⑩ 参数敏感性：两个参数都敏感，但强度差一个量级** —— `maxVolumeRatio` 在**三个** `maxBreakDepthRatio` 固定组里**都**观察到变化且**方向一致单调**（放宽 0.5 → 1.5：交易数 12 → 90；总收益 −39.5200% ~ −5.7606%；最大回撤 5.8560% → 43.5977%；胜率 50% → 30%；盈亏比 0.5294 → 0.3779 ⇒ **放得越松、亏得越多、回撤越大**）；`maxBreakDepthRatio` 敏感指标同样 5 / 5，但在 `maxVolumeRatio = 0.5` 这一固定组内 3 个取值的**交易数完全相同（12）**，只在更松门槛才显出差异（39 → 41、81 → 90）⇒ **其效应远弱于缩量门槛，且被缩量门槛的松紧所掩盖**。⚠️ **负向结论也有证据**：`notVacuous = true`（该参数确有 3 个不同取值 × 3 个含 ≥2 取值的**配对比较组**）⇒ 排除「比较机制空转」。**⑪ 候选参数（纯研究事实描述，非系统评级）**：两轮共 **4 + 9** 个组合**总收益全为负**（第二轮 **0 / 9** 为正），区间 `−39.5200% ~ −5.7606%`；损失最小的组合**一律**在 `maxVolumeRatio = 0.5`（最小门槛）列，且该列对 `maxBreakDepthRatio` **不敏感**（交易数恒 12）—— **但该列仅 12 笔 / 半年窗口，单一窗口单一成本口径，统计上极薄**；**是否值得进入下一阶段验证由人判断**，本轮**不产出**任何 `recommended` / `best` / `optimal`。🔴 **同时观察到一条正向机制证据（非结论）**：门槛越松 ⇒ 交易数与回撤**单调**上升（12 → 90；5.8560% → 43.5977%）⇒ 「缩量」这条筛选**确实在剔除样本**（不是形同虚设）。
>
> **⑥ 质量闸 / 台账 / 非阻塞项**：`tsc --noEmit` **0 error**；定向 `vitest`（`tests/server/runWorkBenchAssembly` + `closedLoopBacktestRun/summary` + `research/conditionSignal/compile` + `research/strategyPersistence` + `client/src/adapters/strategyAdapter`）**9 文件 / 167 例全绿 / exit 0**；`pnpm run test:changed` 的失败**全部落在基线 7 文件之内**（`limitUp.test.ts` 1 + `limitUp.watch.test.ts` 4，环境依赖）⇒ **零新增失败文件**；报告 §26 由 `C:/work/sourcecode/_scratch/srb001_closeout_report.py` **从落盘证据现算**并带 3 条自检（幂等 / 注入式 / 关键坐标）全过。⛔ **未新建** `STRATEGY-BRIDGE-CLOSEOUT-001` / `PARAMETER-AUDIT-xxx` / `ROBUSTNESS-AUDIT-xxx` / `OOS-AUDIT-xxx` / `WFA-AUDIT-xxx`。**`NEXT: Parameter Search — Actual Research Run`**（第一轮 smoke + 第二轮 expand 已完成；后续扩大搜索规模 / 换窗口 / 进入稳健性与 OOS 属**真实研究推进**，**不**先做专项审计）。⚠️ **非阻塞项（只记录，不阻塞）**：数值型 `ENUM` 搜索域**不被支持**（`{mode:"ENUM", values:[…]}` 被 `PARAMETER_SEARCH_DOMAIN_UNCOMPILABLE` **响亮拒绝**并提示改用 `INTEGER_RANGE` / `DECIMAL_RANGE` —— **非缺陷**，但非等步长候选值集合**无法直接表达**）；`BD-12` / `BD-10` / `BD-14` / `BD-16` / `BD-18` / `BD-19` / `BD-20` 保持登记**未代补**；本轮新登记 **3 条「判据自身写错」**（**全部是我自己的探针判据，0 条产品缺陷**，均已在探针内标注「为什么错」再修正）。
...
> 🔴 **编号台账（禁「末条 +1」）**：编号已用至 **`9cm`** ⇒ **下一个未占用 = `9cn`**。已完成的 `1~9` 与 `9a~9bh`、以及第 10~27 项中的 ✅ 已完成项，已**全部移入 `ROADMAP-CHANGELOG.md`**（原样搬运）。本项目历来按「**下一个未占用**」取号，**禁按「末条 +1」推算**。

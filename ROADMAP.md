...
> 🔴 铁律：§44 为「覆盖式」状态区（**只保留最近 1 条「上轮实查」**，历史条目在 `ROADMAP-CHANGELOG.md`）；§44.5 为**未完成**队列（编号按「下一个未占用」，已用至 `9az`）；§47 为 append-only 更新记录。
...
> 上轮实查：**2026-09-17 23:10 GMT+8 · 「创建候选点了没反应」两个真因已修 —— 深链静默 return + 13.2 秒无反馈**。用户反馈「这个页面只有一个创建 candidate 的按钮，点了之后没反应」。按 §16 纪律用无头 Chrome + CDP **真的点了那个按钮**取证（`cdp_click_v2.mjs`）。**真因 A（本轮上个补丁引入）**：`ResearchAsk.tsx:276-277` 的 `if (questionId === null || outcome === null) return;` —— 深链走 `?runId=` 时 `questionId` 恒为 null ⇒ **静默返回零反馈**；而服务端 `researchPlannerRouter.ts:463-497` 的 `createCandidate` 内部是 `buildResearchOutcome(repos,{questionId?|runId?})`、**两者皆收** ⇒ 前端身份改为**择一**（`questionId ?? runId`）+ 静默 return 换成 `toast.error`。**真因 B（原有 UX 缺陷）**：修 A 后重测拿到完整成功路径 —— `postData` 含 `runId:750003` + `patternId:first-limit-pullback-hold-shrink`、响应 **HTTP 200**、**`succeededAtMs = 13200`（真实耗时 13.2 秒）**、toast「已创建候选草稿 #930003」；13 秒来自服务端跑完整 `buildResearchOutcome` 聚合（28 条分析）⇒ 而按钮 pending 时**只 disabled、文案不变**（始终「创建 Candidate」）⇒ 用户仍会以为坏了 ⇒ 改为文案 + spinner「正在创建候选…（需重建本次结论，约 10~30 秒）」。**附带 C**：`ResearchDetail.tsx` 的 `<Link><Button/></Link>` 渲染成 **`<a><button>` 非法嵌套** ⇒ 按项目既有范式改 `<Button asChild><Link>`（`ResearchList.tsx:87-91`）。**落库核对**：候选 930003 的 `entryRule.extra.recipe.recipeId` / `observationWindow {2,2,TRADING_DAY}` / `trigger FIRST_VALID_DAY` / `filterRule {groups:[]}`（刻意）/ `sourceTraceJson.patternGateSummary` 3 条门槛 / `parameterSpaceJson` 三参数全 TUNABLE —— **全部正确** ⇒ 链路本来就是通的。**清理**：探针点击产生的 3 条 DRAFT（930001/930002/930003）已按 id 精确删除（14→11，残留 0；依据 = `information_schema` 实测无任何表含 `candidateId` 列），备份 `_scratch/step0/backup/candidates_deleted_930001_930003.json`。**验收**：`tsc`=0；全量 8 failed / 17（集合与基线逐项一致 ⇒ 零新增）；`vite build` 成功；行尾 0 漂移；改动面 +208/-13 与 +12/-0。**探针自身踩 3 坑（已写进 skill §16）**：只听 `responseReceived` 会漏「已发出未响应」；等 9 秒取 toast 必空（sonner 4 秒消失）；模板字符串里的正则 `\s` 反斜杠被吃掉。
...
> 🔴 **编号台账（禁「末条 +1」）**：编号已用至 **`9az`** ⇒ **下一个未占用 = `9ba`**。已完成的 `1~9` 与 `9a~9au`、以及第 10~27 项中的 ✅ 已完成项，已**全部移入 `ROADMAP-CHANGELOG.md`**（原样搬运）。本项目历来按「**下一个未占用**」取号，**禁按「末条 +1」推算**。

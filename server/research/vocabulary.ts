/**
 * RESEARCH-EXPERIMENT-003 —— 研究域的**共享词汇与领域类型**。
 *
 * 来源：`server/researchCore/types.ts` 整体搬迁，正文逐字节未改（只补了本段说明）。
 *
 * 为什么保留：这些是**类型层词汇**，仍被在用的研究侧模块消费 ——
 *   - `research/patternLibrary/**`：模式声明库的 `research` 投影用到研究类型 / 分析类型 /
 *     条件运算符 / 结论类型 / 模块能力；
 *   - `research/candidateRules.ts`（策略候选状态机与不变量）与 `research/conditionSet.ts`。
 *
 * 🔴 与搬迁前的关系：`server/researchCore/**` 的**领域实现**
 * （conclusions / findings / hypotheses / results / executionLog 以及 repository 层）
 * 已随 RESEARCH-EXPERIMENT-003 整体删除 —— 旧 Research（Analysis → Finding → Conclusion）
 * 不再有任何读写实现。本文件里对应的**实体接口只是类型定义**，没有任何生产代码
 * 实例化或持久化它们，它们当下的作用仅是「不让既有类型引用断链」；004 评估收敛为最小词表。
 */
/**
...
// ---------------------------------------------------------------------------
// 分段窗统计口径（RESEARCH-EXPERIMENT-003 —— 从已删除的 `researchCore/config.ts` 迁入）
//
// 消费方：前端 `client/src/components/research/createAnalysisForm.ts`（分段窗分析表单的
// `satisfies` 穷尽约束）与 `conditionSet.ts` 的分段关系分析。这是**词表**，不是分析实现。
// ---------------------------------------------------------------------------

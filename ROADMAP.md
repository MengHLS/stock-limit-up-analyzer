...
> 🔴 铁律：§44 为「覆盖式」状态区（**只保留最近 1 条「上轮实查」**，历史条目在 `ROADMAP-CHANGELOG.md`）；§44.5 为**未完成**队列（编号按「下一个未占用」，已用至 `9bm`）；§47 为 append-only 更新记录。
...
> 上轮实查：**2026-09-19 00:42 GMT+8 · 首页（`/`）入口从左栏移到左上角网站标题（用户口述）**（编号 `9bm` / HOMEPAGE-005；无头 Chrome 真机量 DOM `_probe_sidebar_active_highlight.mjs` **PASS 37 / FAIL 0 / SKIP 0**）。**用户原话**：**「首页做成点击左上角网站标题访问，不要单独有一栏在复盘分析下面」**。**交付**：`client/src/components/AppShell.tsx` —— ① 删掉「复盘分析」组首的 `{ label: "首页", path: "/", icon: LayoutDashboard }` 与随之空转的 `LayoutDashboard` import（无其它引用）；② `SidebarHeader` 里**原本就已 `onClick -> "/"`** 的标题按钮补齐**可点击 + 可访问性**（`type="button"`、`title` 与 `aria-label` = 「返回首页」、显式 `cursor-pointer`、`focus-visible:ring-2`）并加稳定锚点 `data-slot="sidebar-home-link"`。**行为**：`/` 路由不变（仍是行情复盘总览 `Dashboard`），本轮只换**入口载体** ⇒ 站在首页时侧栏**不再有任何高亮项**（此前唯一点亮的就是被删掉的「首页」项）；`isPathActive` 对 `/` 的 `currentPath === "/"` 分支**保留不动**。**顺带纠正探针两处存量漂移**：`ROUTES` 里 `/` 早已**不该**期望点亮「涨停复盘」（HOMEPAGE-001 起 `/` 已不是涨停复盘明细页）⇒ 改为 `['/limit-up', '涨停复盘']`；并补登记此前漏掉的 `/research/ask`（提问研究）。**验收**：`tsc --noEmit` = 0；全量 `vitest run` = **8 failed files / 17 failed tests = 基线零新增**；探针 **32 条 → 37 条**（新增 5/6 段三条：首页无侧栏高亮 / 侧栏确无「首页」项 / 标题按钮文案·提示·手型光标；6/6 段一条：从 `/strategies` 点标题回 `/` 且零高亮）。**边界**：纯 `client/**`（+ 一个探针文件）⇒ 零 `server/**` 改动、不热重启、不杀在途 Run；零迁移 / 零新表 / 零新依赖 / 零新端点。
...
> 🔴 **编号台账（禁「末条 +1」）**：编号已用至 **`9bm`** ⇒ **下一个未占用 = `9bn`**。已完成的 `1~9` 与 `9a~9bh`、以及第 10~27 项中的 ✅ 已完成项，已**全部移入 `ROADMAP-CHANGELOG.md`**（原样搬运）。本项目历来按「**下一个未占用**」取号，**禁按「末条 +1」推算**。

...
const projectRoot = resolve(import.meta.dirname, "../../server", "..");
...
describe("独立组合资金回测页面", () => {
  ...
  it("独立页保留参数、资金审计和完整订单表", () => {
    ...
    expect(styleSource).not.toContain("[data-trade-difference-table]");
    expect(pageSource).not.toContain("data-trade-difference-table");

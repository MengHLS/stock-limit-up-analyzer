...
} from "../../../../server/research/factorAblation/variants";
import {
  assessAblationRun,
  resolveAblationThresholds,
} from "../../../../server/research/factorAblation/assess";
import {
...
} from "../../../../server/research/factorAblation/serialize";
import {
  formatAblationRunId,
  generateAblationRunId,
  runAblationAssessment,
} from "../../../../server/research/factorAblation/run";
import { toAblationOfdAssessmentInput, toAblationRobustnessView } from "../../../../server/research/factorAblation/adapt";
import { verifyAblationOosDiscipline } from "../../../../server/research/factorAblation/discipline";
import { assessOfdOverfitting } from "../../../../server/research/overfittingDetection/assess";
import type {
  ...
  AblationRequest,
  AblationTarget,
} from "../../../../server/research/factorAblation/types";
...
describe("⑨ 与 C-19.2 OosIsolationRun 复用对照（真实 walk-forward → OOS 隔离报告派生 OOS 求值）", () => {
  it("OOS evaluator 从真实 OosIsolationRun.report 派生：IS 强贡献 + OOS 无贡献 → 信号触发", async () => {
    ...
    const { runWalkForward } = await import("../../../../server/research/walkForwardRun/run");
    const { generateWalkForwardSplits } = await import("../../../../server/research/walkForwardRun/windows");
    const { runOosIsolation } = await import("../../../../server/research/oosIsolation/run");

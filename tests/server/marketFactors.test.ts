...
// 测试文件已收拢到 tests/；mock 说明符必须与被测源码 import 的模块**解析为同一文件**，
// 否则 mock 到的是 tests/server/tushare（不存在）而非 server/tushare，mock 形同失效。
vi.mock("../../server/tushare", () => ({
  fetchTushareDailyPricesByDate: vi.fn(),
}));

import { fetchTwoMarketTurnoverYi, parseMarginWorkbookYi, parseStoredMarketYi } from "../../server/marketFactors";
import { fetchTushareDailyPricesByDate } from "../../server/tushare";

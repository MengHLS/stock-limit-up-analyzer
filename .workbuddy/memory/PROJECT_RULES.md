...
- **行尾必须逐文件实测**，禁按「除某几文件外都是 LF」推断。实测：`ROADMAP.md`、`ROADMAP-CHANGELOG.md`、`drizzle/schema.ts`、**`client/src/**/*.tsx`（`SentimentAnalysis.tsx` 302/302 CRLF）** 均**纯 CRLF**；`.workbuddy/memory/*.md` 与逐日日志**纯 LF**。🔴 **2026-09-15 起仓库本地 `core.autocrlf=false`**（`.git/config`，覆盖系统级 `etc/gitconfig` 的 `true`）⇒ **工作区行尾 = 磁盘真身，`git checkout --` 不再把 CRLF 洗成 LF、也不再断言整文件重写**（已用 sha256 实测：checkout 前后 `ROADMAP.md` 哈希不变）。⚠️ 但 **HEAD blob 内部仍是 LF**（历史遗留，`git cat-file -p <rev>:<path>` 依然只给 LF）⇒ **判据仍然只能是 `b.count(b"\r\n")` vs `b.count(b"\n")`**；⚠️ `git checkout --` 对**已被 autocrlf 洗成 LF 的文件**不会反向补回 CRLF（单向下行），改前仍须 `git status --short -- <file>` 确认那次 `M` 是自己。

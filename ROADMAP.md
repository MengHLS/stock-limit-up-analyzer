...
> 🔴 铁律：§44 为「覆盖式」状态区（**只保留最近 1 条「上轮实查」**，历史条目在 `ROADMAP-CHANGELOG.md`）；§44.5 为**未完成**队列（编号按「下一个未占用」，已用至 `9bi`）；§47 为 append-only 更新记录。
...
> 上轮实查：**2026-09-19 13:40 GMT+8 · STRATEGY-ARCH-001 Strategy Core 一次性实施（`9bi`）** —— 新增纯域层 `server/strategyCore/**`（20 文件 / 6806 行）+ `tests/server/strategyCore/**`（6 测试文件 + 2 夹具 / 112 用例），把「策略语义」从「某一个 Backtest + 某一个 Dataset」里剥出来：**RuleGraph**（ALL/ANY/NOT/CONDITION/EVENT/WINDOW/SEQUENCE/TRIGGER，且 `A AND (B OR C)`、`A AND NOT(B)`、`A → B → C` 均可表达）· **Temporal/PIT**（相对日代数 + `evaluate(T)` 只读 `≤ T` 的 bar，越界**记录违规**而非静默）· **FeatureRegistry**（新增特征不改 Core；`availability` 改为**相对当前 bar 的**声明，取代 legacy 恒为 `1990-01-01` 的假声明）· **ParameterResolver**（`DERIVED` **真正求值** + 表达式解析 + 循环依赖检测 + 「只有 TUNABLE 可被搜索」唯一权威）· **DataRequirements + Compatibility Contract**（含「Definition 内不得出现 datasetVersionId」的机器可查守卫）· **ExecutionSemantics**（signal/confirmation/execution/price/state 五面，方向规则 E1–E5）· **Capabilities**（由规则图**推导**并与声明**比对**，反「声明了却无效」）· **LeakageGuard**（静态审计 A1–A7 + 运行时关卡 + 产出前断言，三层）· **StrategyRuntime.evaluate** 统一入口 · **StrategyDecision**（events/conditions/signals/entryIntents/exitIntents/positionIntents，不返回任何 Backtest 专用对象）· **Fingerprint**（覆盖行为面、排除 metadata）· **StrategyVersion 不可变**（`updateVersionDefinition` **恒抛** ⇒「没有第二条改内容的路径」可断言）· **StrategyRunSnapshot**（补上 legacy 留档实测全缺的 `parameterSet` / `codeVersion` / `engineVersion` / `seed` / universe / datasetReference）· **legacy 双向 Adapter**（未登记映射一律抛错、不猜）。**收敛方向**：Core 是**语义**权威，legacy `StrategyDefinition` 降级为**存储编码** —— 「legacy → Core → legacy → Core」**指纹逐字节相等**（同义子集可逆，已测）。**零已跟踪文件改动 / 零迁移 / 零新端点 / 零前端改动**（`server/**` 与 `client/**` 既有文件一行未改 ⇒ 无热重启风险、无契约破坏）；验收 `tsc`=0 · 聚焦 112/112 · 全量失败集合与基线**逐项一致（8 文件 / 17 用例，零新增）** · `vite build` 成功 · 行尾 0 漂移。**实测抓出并修掉 4 个 Core 自身缺陷**：特征注册聚合抛错码张冠李戴 · `bar.<派生字段>` 的 availability 漏审（桥接未纳入）· 无 WINDOW 时静态 A4 界编造为 0 而误杀合法定义 · `collectRuleFeatureReferences` 漏掉派生字段桥接（会导致 `featureRequirements` 空声明却在运行时抛 FEATURE_NOT_REGISTERED）。**如实登记未完成**：Core 尚未接入生产 tRPC 路径（`loopRun` / 评估端口仍走 legacy `assembleRunWorkbenchInputs` + `signalEngine`）⇒「旧模型不再作为第二套真相来源」目前只到「语义权威 + 双向适配 + 指纹等值」层面，接产属下一 STEP；阈值型出场（TAKE_PROFIT / STOP_LOSS / TIME_EXIT）需「入场价 / 入场日」这类运行态引用，仍在 `exitRules` 以**声明**形式保留、未进 `exitRuleGraph`。详见 `docs/research/STRATEGY-ARCH-001-IMPLEMENTATION-REPORT.md`。
...
> 🔴 **编号台账（禁「末条 +1」）**：编号已用至 **`9bi`** ⇒ **下一个未占用 = `9bj`**。已完成的 `1~9` 与 `9a~9bh`、以及第 10~27 项中的 ✅ 已完成项，已**全部移入 `ROADMAP-CHANGELOG.md`**（原样搬运）。本项目历来按「**下一个未占用**」取号，**禁按「末条 +1」推算**。

...
} from "../../../../../client/src/components/research/candidateSketchVocabulary";
import {
  SKETCH_BLOCK_LABELS,
  buildSketchPatch,
...
import {
  ...
  matchCostPreset,
} from "../../../../../client/src/components/research/candidateSketchCostPreset";
...
function conditionOperatorKeysFromServerSource(): string[] {
  const file = path.resolve(
    import.meta.dirname, "../../../../../client/src/components/research",
...
describe("校验：错误（填了但不合法）与缺口（转正必填但没填）", () => {
  it("空草稿 → 全是缺口，没有错误（本来就没填，谈不上填错）", () => {
    ...
    expect(result.gaps.join()).toContain("事件类型");
...
describe("界面段（按交易决策顺序）", () => {
  ...
  it("段的顺序就是「下单时的思路」：买什么 → 什么条件买 → 什么时候买 → 怎么卖 → 买多少 → 成本 → 参数", () => {
    expect([...SKETCH_SEGMENT_KEYS]).toEqual([
      "what",
      "condition",
      "when",
      "exit",
  ...
  it("完整草图 → 七段摘要都是人话，且必填段全部「齐了」", () => {
    ...
    // ① 买什么：只剩「观察哪类事件」—— 条件已经归到 ② 什么条件买（见下一条）。
    expect(summarizeSketchSegment(drafts, "what")).toBe("首个涨停（首板） · 3 个事件参数");
    // ② 什么条件买：只有 `filterRule`（这正是它唯一的去向：entry.conditions）。
    expect(summarizeSketchSegment(drafts, "condition")).toBe(
      "事件日当天（rd0）的收盘价 大于等于 5 且 事件日的换手率 不属于 st,delisted",
    );
    // ③ 什么时候买：时点 + 窗口 + 触发，**不再混进条件**。
    expect(summarizeSketchSegment(drafts, "when")).toBe(
      "次一交易日开盘买入 · 观察第 1–5 交易日 · 首个有效日触发",
    );
  ...
  it("空草图 → 摘要基本为空；③ 怎么卖 例外，它会明说「未设置」（那是提醒，不是假摘要）", () => {
    ...
    expect(summarizeSketchSegment(emptySketchDrafts(), "condition")).toBe("");
  });
...
describe("买入条件（filterRule）：语义是「满足才买」，不是「剔除」", () => {
  it("🔴 归属段是 condition（什么条件买），不是 what（买什么）也不是 when（什么时候买）；标签也必须说人话", () => {
    // 依据：`definitionBuild.ts:506-508` 把 filterRule 唯一地送进 `entry.conditions`，
    // 而那是「全部满足才产生买入信号」。
    expect(SKETCH_BLOCK_HOME_SEGMENT.filterRule).toBe("condition");
    expect(sketchSegmentBlocks("condition")).toContain("filterRule");
    expect(sketchSegmentBlocks("what")).not.toContain("filterRule");
    // 「什么时候买」（when）只装 entryRule 的时点/窗口/触发 —— 它把条件放进来正是
    // 原来「还差一箱校验」看不出来的根因，所以这条断言必须钉住。
    expect(sketchSegmentBlocks("when")).not.toContain("filterRule");
  ...
  it("条件段落空 = 出现事件即视为满足，但**不是**转正缺口（entry.conditions 可为空数组）", () => {
    ...
    expect(result.gapDetails.some((item) => item.segment === "condition" && item.label.includes("买入条件"))).toBe(false);
    expect(sketchSegmentStatuses(withoutFilter).find((s) => s.segment === "condition")?.required).toBe(false);
    // 条件只影响 ② 段摘要，**不再渗进 ③ 什么时候买** —— 后者只提时点/窗口/触发。
    expect(summarizeSketchSegment(drafts, "condition")).toContain("事件日当天（rd0）的收盘价");
    expect(summarizeSketchSegment(drafts, "when")).not.toContain("买入条件");
  });
});

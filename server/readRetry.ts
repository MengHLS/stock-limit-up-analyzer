/**
 * PERF — 跨境只读的**有界瞬时错误重试**（通用工具，与任何领域无关）。
 *
 * ## 为什么搬到这里（RESEARCH-EXPERIMENT-002 · P0 解耦）
 *
 * 本模块原先住在 `server/researchEngine/readRetry.ts`。它自己**没有任何领域依赖**
 * （无 import、纯工具），但 `server/db.ts` 与
 * `server/runWorkbenchAssembly/datasetFromRegistry.ts` 都从那里 import 它 ——
 * 于是「整个库的入口」与「生产回测装配链」在**模块加载期**就和旧 Research 目录
 * （`server/researchEngine/**`）挂在一起（002 的 import 图可达性探针实测命中）。
 *
 * 搬家后：逻辑零改动（正文逐字节照搬），`server/researchEngine/readRetry.ts`
 * 保留为 `export * from "../readRetry"` 的转出口（**既有 import 路径零破坏**），
 * 两个生产消费方改指本文件 ⇒ 那条边被切断。
 *
 * ## 为什么需要重试
 *

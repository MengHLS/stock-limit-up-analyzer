...
> 🔴 铁律：§44 为「覆盖式」状态区（**只保留最近 1 条「上轮实查」**，历史条目在 `ROADMAP-CHANGELOG.md`）；§44.5 为**未完成**队列（编号按「下一个未占用」，已用至 `9be`）；§47 为 append-only 更新记录。
...
> 上轮实查：**2026-09-18 13:10 GMT+8 · `parameterSpace` 的三个键被候选草图表单拒收（已修）**。用户报错：「该块含结构化表单表达不了的内容，原样展示：parameterSpace.max_volume_ratio 出现未收录的键：parameterRole、defaultValue、description」。**真因**：前端白名单 `candidateSketchForm.ts:317` 只有 `[type,min,max,step,allowedValues]`，而服务端 `definitionBuild.ts:497/:550` **真的读** `parameterRole`（决定是否进 Parameter Search）与 `defaultValue`（执行层必需，缺了抛 `RECIPE_PARAMETER_NO_DEFAULT`）（`description` 则由服务端 `:595` 自行生成、不读草稿值）⇒ 含这三键的参数行触发 `unknownKeysOf` ⇒ **整块降级为原样展示**。**影响面（真实库实测）**：13 条候选中三键齐全 1 条（`960001`，用户 12:56 创建 —— 反证上一轮「点击没反应」已修）、含 `defaultValue` **9 条** ⇒ 该降级**非本轮引入**，是 `parameterRole` 新加后第一次以三键齐出形式暴露。**严重度 P1**：`buildSketchPatch:1861` 对 raw 块 `continue` ⇒ 不进 patch、**不丢数据**；`gaps` 只用于绿色提示、保存按钮由 `patch.ok` 决定 ⇒ **不阻塞**保存与转正；受伤的是可见性与可编辑性。**修复（4 文件，仅 client/**）**：白名单 5→8 键；`ParameterRowDraft` 加 3 字段 + 解析 + 序列化（**空串一律省略**）+ 校验；可编辑表单加 3 个输入；只读卡片参数表加 3 列。**顺带修正语义错误**：旧文案「min/max 都要给（**参数角色恒为 TUNABLE**）」在角色可声明 `FIXED` 后不成立 ⇒ 改为「先定角色、再按角色决定范围是否必需」，与服务端 `buildParameters:522` 同口径。**取证（无头 CDP）**：`/research/candidates/960001` 渲染出六列表格（参数名/类型/取值域/**角色**/**默认值**/**说明**），`hasRawNotice=false`、零 pageError。**验收**：`tsc`=0；`candidateSketchForm.test.ts` **63/63**（+5）；全量 8 failed / 17（集合与基线逐项一致 ⇒ 零新增）；行尾 0 漂移。⚠️ `CandidateSketchFields.tsx` 实测「工作区 CRLF / HEAD LF」纯行尾漂移（字节差 1298==行数），本轮按 HEAD 归一化为 LF ⇒ numstat 41 行而非 1300+。
...
> 🔴 **编号台账（禁「末条 +1」）**：编号已用至 **`9be`** ⇒ **下一个未占用 = `9bf`**。🔴 `9be`（HOMEPAGE-001）已于 2026-09-18 **就地完成**（其后的 5 条版式追加要求属**同一号内收敛**⇒ **不消耗新号**，故「下一个未占用」仍 = `9bf`）。已完成的 `1~9` 与 `9a~9bd`、以及第 10~27 项中的 ✅ 已完成项，已**全部移入 `ROADMAP-CHANGELOG.md`**（原样搬运）。本项目历来按「**下一个未占用**」取号，**禁按「末条 +1」推算**。

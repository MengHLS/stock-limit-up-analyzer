 ...
 * 受约束组合因子实验组 —— **共用页面**。
 *
 * 三个方案的结果结构完全同构（同一个模板、只差成员与权重），因此**共用一份页面**：
 * 页面从 `descriptor` 取标题、从 `customPayload.composition` 取成员与**实际生效权重**
...
export default function CompositeFactorConstrainedWeightStudyPage({
  descriptor,
  outcome,
}: ExperimentPageProps) {
  ...
  return (
    <div className="space-y-4" data-experiment-page={descriptor.pageKey}>
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-xs">
            ...
            {custom.normalization?.bucketFingerprint ?? "—"}。本组实验只改成员集合与权重口径，
            标准化 / 桶边界 / 方向全部来自冻结契约，组内无边界搜索与权重优化。
          </CardDescription>
        </CardHeader>
      ...
      <GenericExperimentResult
        ...
        reason="使用实验专用页面渲染（顶部为成员与生效权重、主判据摘要）。"

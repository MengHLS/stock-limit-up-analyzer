...
> 上轮实查：**2026-09-16 02:20 GMT+8 · `9ar` / BRIDGE-CONDITION-EXPRESSION-001 修复 —— 条件右值「静默降级」的消灭（声明 ↔ 执行对齐；4 源文件 + 3 测试文件，零迁移 / 零新端点 / 零新依赖）**。触发 = 承接 RESEARCH-STRATEGY-BRIDGE 端到端验收暴露的**已查实缺陷**：已转正 8 条策略中**有 7 条**的 `definition.entry.conditions` 第 2 条恒为 `{field:"bar.volume", operator:"LESS_THAN_OR_EQUAL", value:"prefix.rd0.volume * 0.3", valueType:"CONSTANT"}` —— 拿字符串常量与数值字段比较，与同策略 `recipe` 的真实门槛（`volumeRatio <= max_volume_ratio`）**口径不一致**；用户授权「服务可暂停、不需要页面回测」⇒ 允许改 `server/**`（否则热重启会杀死在途 Run）。**一、根因（两条叠加）**：① **词表缺口** —— `ConditionDefinition.valueType` 只有 `CONSTANT`/`FIELD_REFERENCE`/`PARAMETER_REFERENCE`，**无算术形态** ⇒ `bar.volume <= prefix.rd0.volume * 0.3` 写不出来；② **违纪的静默降级** —— `definitionBuild#inferValueType` 判定顺序是「字段引用文法 → 参数词表 → **其余一律 CONSTANT**」，含算术的右值**原样落成字符串常量**，违反转换器「闭集，不静默忽略／不静默丢弃」纪律。**二、方案选型（否决「加算术表达式文法」）**：给右值加 `EXPRESSION` 需自建解析器 + **运行时求值器**，并同步 `definitionValidation`/`projection`/5 张投影表/legacyViews/前端 ⇒ 爆炸半径最大，且等于在已有「字段引用文法单一权威」之上**再造第二套求值语义**；**改采「派生字段 + 响亮失败」** ⇒ 解析器 / 校验器 / 投影 / 运行时求值**零改动**，声明侧通过与配方**同名**的字段与执行侧天然对齐（正解不是「让声明侧能算」，而是「让声明侧能叫出执行侧已有的名字」）。**三、修法（4 源文件）**：① `strategySchema/definition.ts` 新增 `STRATEGY_DERIVED_BAR_FIELDS = [volumeRatio, haircutFromEventLow, isBullish, momentumFromEventClose]`（**命名唯一真源 = 配方特征 id**，逐条对齐口径：量能比 / 回撤深度（正数=跌破）/ 阳线 0-1 / 相对事件日收盘动量）+ `STRATEGY_CURRENT_BAR_FIELDS = 原始列 ∪ 派生字段`，`isKnownFieldReference` 的 `currentBar` 分支改用后者；**`prefix.*`/`post.*` 仍只认原始列**（派生量只在「当前 bar」成立 —— 白名单语义 = **真实可提供**，不是「名字听起来合理」）；全部落 `CURRENT_BAR` 时间域 ⇒ **按构造不含前视**，与 Dataset `path.*` 同名派生列口径相同但取数语义不同（后者属前视打标签层，禁作信号条件）。② `strategySchema/definitionValidation.ts` **仅**把 `UNKNOWN_FIELD_REFERENCE` 的提示改为展示新白名单（不再教用户写出必被拒的引用）。③ `strategyCandidate/definitionBuild.ts`：`inferValueType` → **`resolveConditionValueType(value, codes, path)`** + 新增 `rejectExpressionAttempt` **响亮失败**（`PROMOTE_SKETCH_INVALID`，**点名路径** + 给出两条改写方向「派生字段 / 参数引用」），判据**只两条**（①以合法字段引用开头但整串非法；②字段引用与算术运算符**同时**出现 ⇒ `"FIRST_LIMIT_UP"`/`"MAIN"`/`"2026-09-01"`/`"T+1"` 一律放行为 `CONSTANT`，单测设对照组锁死），`IN`/`NOT_IN` 数组右值逐元素检查。④ `client/src/components/research/candidateSketchVocabulary.ts` 词表拆分（原始列 / 派生字段）+ **位置感知** choices（`currentBar` 全量、`preEvent`/`forwardBar` 仅原始列，与后端白名单同构）+ 提示语从「**表达不了**」改写为「**现在可以这样表达**」（纪律：禁提供必然被转正拒掉的选项）。**四、向后兼容（关键设计约束）**：`validateCanonicalStrategyDefinition` 在**读路径**被调用（`strategyPersistence/service.ts#validateVersion`）⇒ 新增规则若「拒绝」既有形态，8 条已转正策略将**读不出来**；本方案**只扩白名单**、把拒绝加在 **`definitionBuild`（写路径，仅 Promote）** ⇒ 既有 `CONSTANT` 条件**仍然合法**、读路径零变化；且**无任何运行时求值器读 `entry.conditions`**（执行走 `recipe` 门槛）⇒ `valueType` 消费方仅 converter/validator/projection/legacyViews，本轮**只改 converter**。**五、真库取证（只读 · `_probe_bad_conditions_9ar.mts`）**：**§前** 10 个版本 / 16 条条件（`FIELD_REFERENCE=9` / **`PARAMETER_REFERENCE=0`** / `CONSTANT=7`），7 条「形如算术表达式」残留全部落在 420001~420007（420001/003~007 = `* 0.3`、420002 = `* 0.5`），390001（cand-270001）干净；**`PARAMETER_REFERENCE=0` 与「派生字段声明 0/16」互为印证 —— 作者想写「缩量 ≤ 比例」无路可走，只能写算术式再被降级**。**§后** 把 9 条真库草稿的 `filterRuleJson` **原样**喂**真转换器**重放：**成功 1 / 响亮拒绝 8（7 CONVERTED + 1 ARCHIVED）/ 其他失败 0** —— 唯一成功者右值本就是合法字段引用（不受影响），**静默降级已被消灭**。**§对齐不变量**：配方 `featureIds ⊆ bar.*` 白名单**不可声明者 = 0**。**六、端到端复验（真 tRPC + 真 TiDB · `_e2e_condition_expression_9ar.mts` · 自建自清）**：**ALL PASS / 27 断言 / 0 失败** —— 修正写法 `bar.volumeRatio <= max_volume_ratio` + `bar.haircutFromEventLow <= max_drawdown` + `bar.isBullish >= require_bullish` **转正成功**（`cand-450001`@1.0.0），逐条 `valueType=PARAMETER_REFERENCE`、字段名与**运行时配方** `features[].featureId` **同名**、右值 ∈ 配方参数表 ∩ `resolveParameters` 键；**投影 ↔ Canonical 零漂移**（parameters 3 / entry 3 / exit 3 / execution 1 / datasets 1，参数投影全 `TUNABLE`）；负向把右值改回 `"prefix.rd0.volume * 0.3"` ⇒ **响亮拒绝**（信息含 `filterRule.groups[0].conditions[1].value` + `bar.volumeRatio` + `max_volume_ratio`）、**零 Strategy 数据**、候选保持 `ACCEPTED`；收尾**行数守恒**（9/9/10/8/21 回基线）。**七、验收**：`npx tsc --noEmit` = **0 输出 / exit 0**；3 个聚焦测试文件 **187 passed**；全量 `npx vitest run` = **236 文件通过 / 7 文件失败（243）**、**4045 例通过 / 16 失败（4061）**，失败集合与既有基线**逐字一致**（`dataHealth`/`image.uploadAndRecognize`/`limitUp`/`limitUp.watch`/`marketData`/`tushare.secret`/`tushareTradingCalendar`）⇒ **零回归**。**八、边界与遗留**：`drizzle/**` 零改动（**零迁移**）、`shared/**` 零改动、零新端点、零新依赖、`server/**` 文件数仍 **553**；**未**给 `valueType` 加算术形态 ⇒ 未触碰 `projection.ts`/5 张投影表/运行时求值/legacyViews。⚠️ **既有 7 条策略的落库声明未回填**（8 条已转正里 `cand-270001` 本就干净、无需处理；该表内容一经写入禁 UPDATE —— 改内容须新建版本，是 §8 铁律）⇒ 这 7 条声明仍是旧写法，正解是**按新写法重新转正**，而重放已证明旧写法会**响亮失败**（把改写责任交回作者，不替它猜）；派生字段**只登记「执行侧已实现」的 4 个**（`momentumFromEventClose` 目前仅用于排序），**不是愿望清单**。报告 `docs/research/BRIDGE-CONDITION-EXPRESSION-001-implementation.md`。**状态 COMPLETE**。
...
> 🔴 **编号台账（禁「末条 +1」）**：编号已用至 **`9bc`** ⇒ **下一个未占用 = `9bd`**。已完成的 `1~9` 与 `9a~9bc`、以及第 10~27 项中的 ✅ 已完成项，已**全部移入 `ROADMAP-CHANGELOG.md`**（原样搬运）。本项目历来按「**下一个未占用**」取号，**禁按「末条 +1」推算**。
